...
try {
  ...
  if(!nativeOnly)await send('Profiler.start',{},sessionId);
  const measurements=await evaluate('window.doProfile()',sessionId);
  const {profile}=nativeOnly?{profile:{nodes:[],samples:[]}}:await send('Profiler.stop',{},sessionId);
  const hashes={};
  ...
  if(!nativeOnly)await fs.writeFile(path.join(out,'profile.cpuprofile'),JSON.stringify(profile)+'\n');
  const result={scope:'instrumented profiling only; elapsed times are not confirmation speed evidence',nativeOnly,date:new Date().toISOString(),version,host:{arch:os.arch(),release:os.release(),cpu:os.cpus()[0]?.model},args,hashes,loadsBefore,loadsAfter:os.loadavg(),validation,measurements};
