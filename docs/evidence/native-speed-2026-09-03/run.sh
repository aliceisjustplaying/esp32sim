...
output_dir="${1:-$receipt_dir/raw}"
expected_build="/Users/sarah/Archives/esp32s3/pinned-builds/tinydraw-vector-v2-9cb651e0"
...
if [[ "$build_root" != "$expected_build" ]]; then
  echo "TINYDRAW_VECTOR_V2_BUILD must be $expected_build" >&2
  exit 2
fi
