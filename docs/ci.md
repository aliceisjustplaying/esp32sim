...
`.github/workflows/ci.yml` defines the required proof surfaces for this
fork. It pins Rust 1.98.0 and every action by full commit SHA, uses
...
- `Rust format`: runs `rustfmt --check` over every Rust file lane G added
  or changed.
...
Whole-workspace formatting is not green at the fork base. The exact
receipt is
[`receipts/2026-08-31-rustfmt-baseline.txt`](receipts/2026-08-31-rustfmt-baseline.txt).
At `puck/base` commit `aa851249341e8cd122e7f4852d4c0f002e46d887`,
`cargo fmt --all -- --check` exits 1 with 893 diff hunks across 39 files.
Lane G does not rewrite those product files. Full-workspace Rust formatting
therefore remains an explicit exit blocker pending maintainer disposition.
