...
/// non-overlapping WiFi channels sitting on top of it, wandering a little, plus noise. The board
/// or a script can move the levels (`set_channel_dbm`).
...
impl Ieee802154 {
    ...
    pub fn new() -> Self {
        ...
        Ieee802154 { ram: RegRam::new(), events: 0, event_en: 0, freq: 3, duration: 0, ed_rss: -92, ed_left: None, scans: 0, level_dbm, noise: 0x9e37_79b9 }
    }
    pub fn set_channel_dbm(&mut self, channel: u8, dbm: i8) { if (11..=26).contains(&channel) { self.level_dbm[(channel - 11) as usize] = dbm; } }
    ...
    fn sample(&mut self) -> i8 {
        self.noise ^= self.noise << 13; self.noise ^= self.noise >> 17; self.noise ^= self.noise << 5;
        let jitter = (self.noise % 7) as i8 - 3;
...
impl Device for Ieee802154 {
    ...
    fn next_deadline(&self) -> Option<u64> { self.ed_left }
}
