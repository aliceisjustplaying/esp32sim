...
mod native {
    ...
    impl CodeCache {
        pub fn new(size: usize) -> Option<CodeCache> {
            ...
            let flags = 0x0002 | 0x0020; // PRIVATE | ANONYMOUS
                                         // SAFETY: The null address lets the operating system choose the mapping. The supplied
                                         // length and integer flags require no borrowed memory, and failure is checked below.
            let p = unsafe { mmap(std::ptr::null_mut(), size, 7, flags, -1, 0) };
            if p.addr() == usize::MAX || p.is_null() {
                return None;
            }
            Some(CodeCache {
                base: p.cast::<u8>(),
        ...
        fn write(&mut self, words: &[u32]) -> Option<u32> {
            ...
            #[cfg(target_os = "macos")]
            {
                // SAFETY: This thread owns the exclusive cache borrow while changing the current
                // thread's JIT write-protection state.
                unsafe { pthread_jit_write_protect_np(0) };
            }
            // SAFETY: The bounds check above proves `off` is inside the mapping.
            let dst = unsafe { self.base.add(off) };
            // SAFETY: `dst..dst + bytes` is in the mapping, and the source is a distinct slice of
            // exactly `bytes` bytes. The exclusive cache borrow prevents concurrent writes.
            unsafe { std::ptr::copy_nonoverlapping(words.as_ptr().cast::<u8>(), dst, bytes) };
            #[cfg(target_os = "macos")]
            {
                // SAFETY: This balances the write-enable call above on the same thread.
                unsafe { pthread_jit_write_protect_np(1) };
                // SAFETY: `dst..dst + bytes` is the initialized code range published by this write.
                unsafe { sys_icache_invalidate(dst.cast::<c_void>(), bytes) };
            }
            #[cfg(target_os = "linux")]
            {
                // SAFETY: The bounds check above proves the end is in or one past the mapping.
                let end = unsafe { self.base.add(off + bytes) };
                // SAFETY: `dst..end` is the initialized code range published by this write.
                unsafe { __clear_cache(dst.cast::<c_void>(), end.cast::<c_void>()) };
            }
            self.used += bytes;
            Some(off as u32)
        }
    ...
    const H_READ8: u32 = 0;
    const H_READ16: u32 = 8;
    const H_READ32: u32 = 16;
    const H_WRITE8: u32 = 24;
    const H_WRITE16: u32 = 32;
    const H_WRITE32: u32 = 40;
    const H_EXEC: u32 = 48;
    const H_RAISE_MEM: u32 = 56;
    const H_OVERFLOW: u32 = 64;
    ...
    macro_rules! read_helper {
        ($name:ident, $f:ident) => {
            extern "C" fn $name<B: Bus>(bus: *mut B, addr: u32, pc: u32) -> u64 {
                ...
                bus.note_pc(pc);
                match bus.$f(addr) {
                    Ok(v) => v as u64 | (bus.block_break() as u64) << 33,
                    Err(_) => 1 << 32,
                }
            }
        };
    ...
    extern "C" fn h_exec<B: Bus>(
        ...
        // SAFETY: Compiled blocks pass the exclusive bus pointer supplied to `run`, and this
        // helper returns before generated code resumes.
        let bus = unsafe { &mut *bus };
        // SAFETY: The instruction points into the block arena, which remains stable for the
        // duration of `run`.
        let i = unsafe { &*insn };
        cpu.pc = pc;
        bus.note_pc(pc);
    ...
    impl Helpers {
        pub fn new<B: Bus>() -> Helpers {
            Helpers {
                read8: (h_read8::<B> as *const ()).addr(),
                read16: (h_read16::<B> as *const ()).addr(),
                read32: (h_read32::<B> as *const ()).addr(),
                write8: (h_write8::<B> as *const ()).addr(),
                write16: (h_write16::<B> as *const ()).addr(),
                write32: (h_write32::<B> as *const ()).addr(),
                exec: (h_exec::<B> as *const ()).addr(),
                raise_mem: (h_raise_mem as *const ()).addr(),
                overflow: (h_overflow as *const ()).addr(),
            }
        }
    }
    ...
    const LOFF: Reg = 28;
    const FRAME: i32 = 112;
    const BUDGET_SLOT: u32 = 96;
    const IDX: Reg = 17;
    const EXIT_END: u32 = 0;
    const EXIT_LEFT: u32 = 1;
    const EXIT_TRAP: u32 = 2;
    const EXIT_CUT: u32 = 3;
    const EXIT_TRAP_PRE: u32 = 4;
    ...
    impl<'a> Gen<'a> {
        fn ld_ar(&mut self, dst: Reg, n: u8) {
            self.a.add_imm(IDX, WB4, n as u32);
            self.a.and_mask(IDX, IDX, 6, 0);
            self.a.ldr_idx(dst, AR, IDX);
        }
        fn st_ar(&mut self, n: u8, src: Reg) {
            self.a.add_imm(IDX, WB4, n as u32);
            self.a.and_mask(IDX, IDX, 6, 0);
            self.a.str_idx(src, AR, IDX);
        }
        fn set_pc(&mut self, pc: u32) {
            self.a.mov32(9, pc);
            self.a.str(9, CPU, OFF_PC);
        }
        fn call(&mut self, h: u32) {
            self.a.ldr_x(9, HELP, h);
            self.a.blr(9);
        }
        ...
        fn left_stub(&mut self, pc: u32) -> Label {
            ...
            self.stubs.push(Box::new(move |a: &mut Asm| {
                a.bind(l);
                a.mov32(9, pc);
                a.b(tail);
            }));
        ...
        fn load_loff(&mut self) {
            self.a.mov32(9, self.pc0);
            self.a.sub(LOFF, LEND, 9);
        }
        ...
        fn tlb_probe(&mut self, addr: Reg, size: u32, slow: Label) {
            ...
            self.a.lsr_imm(9, addr, 16);
            self.a.eor_lsr(9, 9, addr, 24);
            self.a.and_mask(9, 9, 9, 0);
            self.a.add_x_lsl(9, TLB, 9, 5); // 32-byte entries
            self.a.ldr(10, 9, 0);
            self.a.ldr(11, 9, 4); // lo, hi
            self.a.cmp(addr, 10);
            self.a.b_cond(Cond::Lo, slow);
            self.a.add_imm(13, addr, size);
            self.a.cmp(13, 11);
            self.a.b_cond(Cond::Hi, slow);
            self.a.sub(10, addr, 10); // offset
            self.a.ldr_x(12, 9, 8); // base
        }
        fn reload_after_helper(&mut self) {
            self.a.ldr(WB4, CPU, OFF_WB);
            self.a.lsl_imm(WB4, WB4, 2);
            self.a.ldr(LEND, CPU, OFF_LEND);
            self.load_loff();
        }
    }
    ...
    pub fn compile(
        ...
        if cc.remaining() < MAX_BLOCK_CODE {
            return None;
        }
        let mut a = Asm::new();
        let (exit, exit_trap, exit_trap_pre, exit_left) =
        ...
        a.mov_x(HELP, 2);
        a.mov(LEFT, 3);
        a.str(3, SP, BUDGET_SLOT);
        a.mov_x(TLB, 5);
        a.mov_x(PVER, 6);
        a.add_imm_x(AR, CPU, OFF_AR);
        a.ldr(WB4, CPU, OFF_WB);
        a.lsl_imm(WB4, WB4, 2);
        ...
        for block_insn in insns.iter_mut() {
            ...
            if max_ar >= 4 && (max_ar / 4) as u32 > checked_w {
                ...
                let ovf = g.a.label();
                let back = g.a.label();
                g.a.ldr(9, CPU, OFF_WS);
                g.a.orr_lsl(9, 9, 9, 16);
                g.a.add_imm(10, WB4, 4);
                g.a.lsr_imm(10, 10, 2);
                g.a.lsrv(9, 9, 10);
                g.a.tst_mask(9, w, 0);
                g.a.b_cond(Cond::Ne, ovf);
                g.a.bind(back);
            ...
            use Op::*;
            match i.op {
                Nop | NopN | Isync | Rsync | Esync | Dsync | Excw | Memw | Extw | Dpfr | Dpfw
                | Dpfro | Dpfwo | Dhwb | Dhwbi | Dhi | Dii | Ipf | Ihi | Iii | Ipfl | Ihu | Iiu
                | Dpfl | Dhu | Diu => {}
                Movi => {
                    g.a.mov32(11, immu);
                    g.st_ar(t, 11);
                ...
                Sext => {
                    g.ld_ar(9, s);
                    g.a.sbfx(11, 9, 0, immu + 1);
                    g.st_ar(r, 11);
                }
                Slli => {
                    g.ld_ar(9, s);
                    g.a.lsl_imm(11, 9, immu & 31);
                    g.st_ar(r, 11);
                }
                Srai => {
                    g.ld_ar(10, t);
                    g.a.asr_imm(11, 10, immu & 31);
                    g.st_ar(r, 11);
                }
                Srli => {
                    g.ld_ar(10, t);
                    g.a.lsr_imm(11, 10, immu & 31);
                    g.st_ar(r, 11);
                }
                Ssr => {
                    g.ld_ar(9, s);
                    g.a.and_mask(9, 9, 5, 0);
                    g.a.str(9, CPU, OFF_SAR);
                }
                Ssl => {
                    g.ld_ar(9, s);
                    g.a.and_mask(9, 9, 5, 0);
                ...
                Ssai => {
                    g.a.movz(9, immu & 31, 0);
                    g.a.str(9, CPU, OFF_SAR);
                }
                Sll => {
                    g.ld_ar(10, s);
                    g.a.ldr(9, CPU, OFF_SAR);
                    g.a.movz(12, 32, 0);
                    g.a.sub(9, 12, 9);
                    g.a.and_mask(9, 9, 6, 0);
                    g.a.lslv(11, 10, 9);
                    g.a.cmp_imm(9, 32);
                    g.a.csel(11, ZR, 11, Cond::Hs);
                    g.st_ar(r, 11);
                }
                Srl => {
                    g.ld_ar(10, t);
                ...
                Sra => {
                    ...
                    g.a.ldr(9, CPU, OFF_SAR);
                    g.a.asrv(11, 10, 9);
                    g.a.asr_imm(12, 10, 31);
                    g.a.cmp_imm(9, 32);
                    g.a.csel(11, 12, 11, Cond::Hs);
                    g.st_ar(r, 11);
                }
                Src => {
                    g.ld_ar(9, s);
                    g.ld_ar(10, t);
                    g.a.ldr(12, CPU, OFF_SAR);
                    g.a.and_mask(12, 12, 6, 0);
                    g.a.lsl_imm_x(11, 9, 32);
                    g.a.orr_x(11, 11, 10);
                    g.a.lsrv_x(11, 11, 12);
                    g.st_ar(r, 11);
                }
                ...
                Moveqz | Movnez | Movltz | Movgez => {
                    ...
                    g.ld_ar(10, t);
                    match i.op {
                        Moveqz => g.a.cbnz(10, skip),
                        Movnez => g.a.cbz(10, skip),
                        Movltz => g.a.tbz(10, 31, skip),
                        _ => g.a.tbnz(10, 31, skip),
                    }
                    g.ld_ar(9, s);
                    g.st_ar(r, 9);
                    g.a.bind(skip);
                ...
                Movf | Movt => {
                    ...
                    if i.op == Movf {
                        g.a.tbnz(9, t as u32, skip);
                    } else {
                        g.a.tbz(9, t as u32, skip);
                    }
                    g.ld_ar(9, s);
                    g.st_ar(r, 9);
                    g.a.bind(skip);
                ...
                Salt | Saltu => {
                    ...
                    g.ld_ar(10, t);
                    g.a.cmp(9, 10);
                    g.a.cset(11, if i.op == Salt { Cond::Lt } else { Cond::Lo });
                    g.st_ar(r, 11);
                }

                // ---- loads and stores through the bus helpers
                L8ui | L16ui | L16si | L32i | L32iN | L32ai | L32r => {
                    let (h, size) = match i.op {
                        L8ui => (H_READ8, 1),
                        L16ui | L16si => (H_READ16, 2),
                        _ => (H_READ32, 4),
                    };
                    ...
                    if i.op == L32r {
                        ...
                        g.ld_ar(1, s);
                        g.a.add_imm32(1, 1, immu, 9);
                    }
                    let (slow, done) = (g.a.label(), g.a.label());
                    if fast {
                        g.tlb_probe(1, size, slow); // x12 = entry base, w10 = offset
                        match i.op {
                            L8ui => g.a.ldrb_u(0, 12, 10),
                            L16ui => g.a.ldrh_u(0, 12, 10),
                            L16si => g.a.ldrsh_u(0, 12, 10),
                            _ => g.a.ldr_u(0, 12, 10),
                        }
                        g.st_ar(t, 0);
                        g.a.movz(12, 0, 0);
                    ...
                    g.stubs.push(Box::new(move |a: &mut Asm| {
                        ...
                        // st_ar(t, w0)
                        a.add_imm(IDX, WB4, t as u32);
                        a.and_mask(IDX, IDX, 6, 0);
                        a.str_idx(0, AR, IDX);
                        a.b(done);
                        a.bind(fault); // recompute the address: nothing changed
                        a.mov_x(0, CPU);
                        a.mov32(1, exc::LOAD_PROHIBITED);
                        if op == Op::L32r {
                            a.mov32(2, immu);
                        } else {
                            a.add_imm(IDX, WB4, s as u32);
                            a.and_mask(IDX, IDX, 6, 0);
                            a.ldr_idx(2, AR, IDX);
                            a.add_imm32(2, 2, immu, 9);
                        }
                        a.mov32(3, pc);
                        a.ldr_x(9, HELP, H_RAISE_MEM);
                        a.blr(9);
                        a.b(tr);
                    }));
                    g.a.bind(done);
                ...
                S8i | S16i | S32i | S32iN | S32ri => {
                    ...
                    g.ld_ar(1, s);
                    g.a.add_imm32(1, 1, immu, 9); // w1 = address, w2 = value
                    g.ld_ar(2, t);
                    let (slow, done) = (g.a.label(), g.a.label());
                    if fast {
                        g.tlb_probe(1, size, slow); // x12 = entry base, w10 = offset, x9 = entry
                        g.a.ldr(11, 9, 20);
                        g.a.cbz(11, slow); // writable?
                        ...
                        g.a.and_mask(13, 10, 8, 0);
                        g.a.sub_imm(13, 13, 3);
                        g.a.cmp_imm(13, 253 - size);
                        g.a.b_cond(Cond::Hi, slow);
                        match i.op {
                            S8i => g.a.strb_u(2, 12, 10),
                            S16i => g.a.strh_u(2, 12, 10),
                            _ => g.a.str_u(2, 12, 10),
                        }
                        g.a.ldr(11, 9, 16);
                        g.a.add_lsr(11, 11, 10, 8); // vbase + (offset >> 8)
                        g.a.ldr_idx(13, PVER, 11);
                        g.a.add_imm(13, 13, 1);
                        g.a.str_idx(13, PVER, 11);
                        g.a.movz(12, 0, 0);
                        g.a.b(done);
                    } else {
                        g.a.b(slow);
                    ...
                    g.stubs.push(Box::new(move |a: &mut Asm| {
                        ...
                        a.bind(fault);
                        a.mov_x(0, CPU);
                        a.mov32(1, exc::STORE_PROHIBITED);
                        a.add_imm(IDX, WB4, s as u32);
                        a.and_mask(IDX, IDX, 6, 0);
                        a.ldr_idx(2, AR, IDX);
                        a.add_imm32(2, 2, immu, 9);
                ...
                | Bgeui | Bnone | Bany | Ball | Bnall | Beq | Bne | Blt | Bge | Bltu | Bgeu
                | Bbc | Bbs | Bbci | Bbsi | Bf | Bt => {
                    let tk = g.left_stub(immu);
                    match i.op {
                        Beqz | BeqzN => {
                            g.ld_ar(9, s);
                        ...
                        Bnone | Bany => {
                            ...
                            g.ld_ar(10, t);
                            g.a.tst(9, 10);
                            g.a.b_cond(if i.op == Bnone { Cond::Eq } else { Cond::Ne }, tk);
                        }
                        Ball | Bnall => {
                            g.ld_ar(9, s);
                            g.ld_ar(10, t);
                        ...
                        Bbci | Bbsi => {
                            g.ld_ar(9, s);
                            if i.op == Bbci {
                                g.a.tbz(9, imm2 as u32, tk);
                            } else {
                                g.a.tbnz(9, imm2 as u32, tk);
                            }
                        }
                        _ => {
                            g.a.ldr(9, CPU, OFF_BR);
                            if i.op == Bf {
                                g.a.tbz(9, s as u32, tk);
                            } else {
                                g.a.tbnz(9, s as u32, tk);
                            }
                        }
                    }
                }
            ...
            if fell_through {
                let cont = g.a.label();
                g.a.cmp_imm(LOFF, next.wrapping_sub(pc0));
                g.a.b_cond(Cond::Ne, cont);
                g.a.ldr(9, CPU, OFF_LCOUNT);
                g.a.cbz(9, cont);
                let ls = g.loop_shared;
                g.a.b(ls);
                g.a.bind(cont);
                if let Some(f) = flag {
                    let nx = g.left_stub(next);
                    g.a.cbnz(f, nx);
                }
            }
            pc = next;
        ...
        g.set_pc(pc);
        g.a.movz(0, EXIT_END, 0);
        g.a.b(exit);

        // shared exits
        g.a.bind(exit_trap);
        g.a.movz(0, EXIT_TRAP, 0);
        g.a.b(exit);
        g.a.bind(exit_trap_pre);
        g.a.movz(0, EXIT_TRAP_PRE, 0);
        g.a.b(exit);
        g.a.bind(exit_left);
        g.a.movz(0, EXIT_LEFT, 0);
        g.a.b(exit);
        g.a.bind(loop_shared); // lcount != 0 and next == lend
        g.a.ldr(9, CPU, OFF_LCOUNT);
        g.a.sub_imm(9, 9, 1);
        g.a.str(9, CPU, OFF_LCOUNT);
        g.a.ldr(9, CPU, OFF_LBEG);
        g.a.bind(left_w9);
        g.a.str(9, CPU, OFF_PC);
        g.a.movz(0, EXIT_LEFT, 0);
        g.a.b(exit);
        g.a.bind(cut_w9);
        g.a.str(9, CPU, OFF_PC);
        g.a.movz(0, EXIT_CUT, 0);
        g.a.b(exit);
        g.a.bind(exit);
        g.a.ldr(9, SP, BUDGET_SLOT);
        g.a.sub(9, 9, LEFT);
        g.a.orr_lsl(0, 9, 0, 16);
        g.a.ldp(27, 28, SP, 80);
        g.a.ldp(25, 26, SP, 64);
        g.a.ldp(23, 24, SP, 48);
        g.a.ldp(21, 22, SP, 32);
        ...
        g.a.ret();
        for s in std::mem::take(&mut g.stubs) {
            s(&mut g.a);
        }
    ...
    pub unsafe fn run<B: Bus>(
        ...
        let f: extern "C" fn(
            ...
            *mut u32,
            ...
            // SAFETY: The caller guarantees that `code` names a compiled entry point in this cache
            // with the declared ABI and concrete bus type.
    ...
    pub const CODE_END: u32 = EXIT_END;
    pub const CODE_LEFT: u32 = EXIT_LEFT;
...
mod native {
    ...
    pub struct CodeCache;
    impl CodeCache {
        pub fn new(_: usize) -> Option<CodeCache> {
            None
        }
        pub fn remaining(&self) -> usize {
            0
