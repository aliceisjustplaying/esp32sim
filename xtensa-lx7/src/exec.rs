...
impl Cpu {
    ...
    fn rotate(&mut self, n: i32) {
        self.windowbase = (self.windowbase as i32 + n).rem_euclid(NUM_WINDOWS as i32) as u32;
    }
    ...
    pub fn raise(&mut self, cause: u32) -> Trap {
        ...
        if self.excm() {
            ...
            self.epc[1] = self.pc;
            self.ps |= ps::EXCM;
            self.pc = self.vecbase.wrapping_add(if self.ps & ps::UM != 0 {
                vec::USER
            } else {
                vec::KERNEL
            });
    ...
    pub fn raise_mem(&mut self, cause: u32, vaddr: u32) -> Trap {
        self.excvaddr = vaddr;
        self.raise(cause)
    }
    ...
    pub fn check_interrupts_pending(&self) -> u32 {
        ...
        let mask_level = if self.excm() {
            self.intlevel().max(EXCM_LEVEL)
        } else {
            self.intlevel()
        };
    ...
    pub fn check_interrupts(&mut self) -> Option<Trap> {
        ...
        if pending & INT_ABOVE[mask_level as usize] == 0 {
            return None;
        }
        ...
        if level == 1 {
            ...
            self.pc = self.vecbase.wrapping_add(match level {
                2 => vec::LEVEL2,
                3 => vec::LEVEL3,
                4 => vec::LEVEL4,
                5 => vec::LEVEL5,
    ...
    pub(crate) fn check_overflow(&mut self, max_ar: u8) -> Option<Trap> {
        if max_ar < 4 || !self.woe() || self.excm() {
            return None;
        }
        ...
        for n in 1..=w {
            if self.windowstart & bit(self.windowbase + n) != 0 {
                ...
                let size = if self.windowstart & bit(m + 1) != 0 {
                    1
                } else if self.windowstart & bit(m + 2) != 0 {
                    2
                } else {
                    3
                };
                self.pc = self.vecbase.wrapping_add(match size {
                    1 => vec::WINDOW_OF4,
                    2 => vec::WINDOW_OF8,
                    _ => vec::WINDOW_OF12,
                });
    ...
    pub fn advance_ccount(&mut self, cycles: u32) {
        ...
        for (i, interrupt) in TIMER_INTERRUPT.iter().enumerate() {
    ...
    pub fn read_sr(&mut self, n: u32) -> Option<u32> {
        Some(match n {
            sr::LBEG => self.lbeg,
            sr::LEND => self.lend,
            sr::LCOUNT => self.lcount,
            sr::SAR => self.sar,
            sr::BR => self.br,
            sr::SCOMPARE1 => self.scompare1,
            sr::ACCLO => self.acclo,
            sr::ACCHI => self.acchi,
            32..=35 => self.m[(n - 32) as usize],
            sr::WINDOWBASE => self.windowbase,
            sr::WINDOWSTART => self.windowstart,
            sr::IBREAKENABLE => self.ibreakenable,
            sr::MEMCTL => self.memctl,
            sr::ATOMCTL => self.atomctl,
            sr::DDR => self.ddr,
            128 | 129 => self.ibreaka[(n - 128) as usize],
            144 | 145 => self.dbreaka[(n - 144) as usize],
            160 | 161 => self.dbreakc[(n - 160) as usize],
            sr::CONFIGID0 => self.configid[0],
            sr::CONFIGID1 => self.configid[1],
            177..=183 => self.epc[(n - 176) as usize],
            sr::DEPC => self.depc,
            194..=199 => self.eps[(n - 192) as usize],
            209..=215 => self.excsave[(n - 208) as usize],
            sr::CPENABLE => self.cpenable,
            sr::INTERRUPT => self.interrupt,
            sr::INTENABLE => self.intenable,
            sr::PS => self.ps,
            sr::VECBASE => self.vecbase,
            sr::EXCCAUSE => self.exccause,
            sr::DEBUGCAUSE => self.debugcause,
            sr::CCOUNT => self.ccount,
            sr::PRID => self.prid,
            sr::ICOUNT => self.icount,
            sr::ICOUNTLEVEL => self.icountlevel,
            sr::EXCVADDR => self.excvaddr,
    ...
    pub fn write_sr(&mut self, n: u32, v: u32) -> Option<()> {
        match n {
            sr::LBEG => self.lbeg = v,
            sr::LEND => self.lend = v,
            sr::LCOUNT => self.lcount = v,
            sr::SAR => self.sar = v & 0x3f,
            sr::BR => self.br = v & 0xffff,
            sr::SCOMPARE1 => self.scompare1 = v,
            sr::ACCLO => self.acclo = v,
            sr::ACCHI => self.acchi = v & 0xff,
            32..=35 => self.m[(n - 32) as usize] = v,
            sr::WINDOWBASE => self.windowbase = v & (NUM_WINDOWS - 1),
            sr::WINDOWSTART => self.windowstart = v & 0xffff,
            sr::IBREAKENABLE => self.ibreakenable = v & 3,
            sr::MEMCTL => self.memctl = v,
            sr::ATOMCTL => self.atomctl = v,
            sr::DDR => self.ddr = v,
            128 | 129 => self.ibreaka[(n - 128) as usize] = v,
            144 | 145 => self.dbreaka[(n - 144) as usize] = v,
            160 | 161 => self.dbreakc[(n - 160) as usize] = v,
            177..=183 => self.epc[(n - 176) as usize] = v,
            sr::DEPC => self.depc = v,
            194..=199 => self.eps[(n - 192) as usize] = v,
            209..=215 => self.excsave[(n - 208) as usize] = v,
            sr::CPENABLE => self.cpenable = v & 0xff,
            sr::INTSET => self.interrupt |= v & INTTYPE_SOFTWARE,
            sr::INTCLEAR => {
                self.interrupt &= !(v & (INTTYPE_SOFTWARE | INTTYPE_EDGE | INTTYPE_PROFILING))
            }
            sr::INTENABLE => self.intenable = v,
            sr::PS => self.ps = v & 0x0007_ff3f,
            sr::VECBASE => self.vecbase = v & !0x3ff,
            sr::EXCCAUSE => self.exccause = v & 0x3f,
            sr::DEBUGCAUSE => {}
            sr::CCOUNT => self.ccount = v,
            sr::PRID => {}
            sr::ICOUNT => self.icount = v,
            sr::ICOUNTLEVEL => self.icountlevel = v & 0xf,
            sr::EXCVADDR => self.excvaddr = v,
            240..=242 => {
    ...
    fn read_ur(&self, n: u32) -> Option<u32> {
        Some(match n {
            0 | 1 => self.accx[n as usize],
            2..=6 => self.qacc_h[(n - 2) as usize],
            7..=11 => self.qacc_l[(n - 7) as usize],
            12 => self.gpio_out,
            13 => self.sar_byte,
            14 => self.fft_bit_width,
            15..=18 => self.ua_state[(n - 15) as usize],
            231 => self.threadptr,
            232 => self.fcr,
            233 => self.fsr,
            _ => return None,
    ...
    fn write_ur(&mut self, n: u32, v: u32) -> Option<()> {
        match n {
            0 | 1 => self.accx[n as usize] = v,
            2..=6 => self.qacc_h[(n - 2) as usize] = v,
            7..=11 => self.qacc_l[(n - 7) as usize] = v,
            12 => self.gpio_out = v,
            13 => self.sar_byte = v,
            14 => self.fft_bit_width = v,
            15..=18 => self.ua_state[(n - 15) as usize] = v,
            231 => self.threadptr = v,
            232 => self.fcr = v & 0x7f,
            233 => self.fsr = v & 0xfff80,
            _ => return None,
        }
        Some(())
    }
...
pub(crate) fn max_ar(i: &Insn) -> u8 {
    ...
    match i.op {
        Add | AddN | Sub | Addx2 | Addx4 | Addx8 | Subx2 | Subx4 | Subx8 | And | Or | Xor | Min
        | Max | Minu | Maxu | Moveqz | Movnez | Movltz | Movgez | Src | Mull | Muluh | Mulsh
        | Mul16u | Mul16s | Quou | Quos | Remu | Rems | Salt | Saltu | AddiN | Lsx | Ssx | Lsxp
        | Ssxp => r.max(s).max(t),
        Movf | Movt | Sext | Clamps | Slli | Sll => r.max(s),
        Neg | Abs | Extui | Srai | Srli | Srl | Sra | Nsa | Nsau | Movsp | L32e | S32e | S32nb
        | Rer | Wer | Mov | MovN | Witlb | Wdtlb | Ritlb0 | Ritlb1 | Rdtlb0 | Rdtlb1 | Pitlb
        | Pdtlb => r.max(s).max(t),
        ...
        Callx8 => s.max(8),
        Callx12 => s.max(12),
        MoviN | Ssr | Ssl | Ssa8l | Ssa8b | Entry | Jx | Callx0 | Iitlb | Idtlb | Beqz | Bnez
        | Bltz | Bgez | BeqzN | BnezN | Beqi | Bnei | Blti | Bgei | Bltui | Bgeui | Bbci | Bbsi
        | Loop | Loopnez | Loopgtz | Lsi | Ssi | Lsip | Ssip | FloatS | UfloatS | Wfr | Dpfr
        | Dpfw | Dpfro | Dpfwo | Dhwb | Dhwbi | Dhi | Dii | Ipf | Ihi | Iii | Ipfl | Ihu | Iiu
        | Dpfl | Dhu | Diu => s,
        L8ui | L16ui | L16si | L32i | L32iN | L32ai | S8i | S16i | S32i | S32iN | S32ri
        | S32c1i | Addi | Addmi | Bnone | Beq | Blt | Bltu | Ball | Bbc | Bany | Bne | Bge
        | Bgeu | Bnall | Bbs | MoveqzS | MovnezS | MovltzS | MovgezS => s.max(t),
        L32r => t,
        Mac16 => s.max(t),
        Pie => i.imm2 as u8,
        _ => 0,
...
fn sat_i32(v: f32) -> u32 {
    if v.is_nan() {
        0x8000_0000
    } else if v >= f32::from_bits(0x4f00_0000) {
        0x7fff_ffff
    } else if v <= -f32::from_bits(0x4f00_0000) {
        0x8000_0000
    } else {
...
fn sat_u32(v: f32) -> u32 {
    if v.is_nan() || v >= f32::from_bits(0x4f80_0000) {
        0xffff_ffff
    } else if v <= 0.0 {
        0
    } else {
...
macro_rules! ld {
    ($cpu:expr, $bus:expr, $f:ident, $addr:expr) => {
        match $bus.$f($addr) {
            Ok(v) => v as u32,
            Err(_) => return Err($cpu.raise_mem(exc::LOAD_PROHIBITED, $addr)),
        }
    };
}
macro_rules! st {
    ($cpu:expr, $bus:expr, $f:ident, $addr:expr, $v:expr) => {
        if $bus.$f($addr, $v).is_err() {
            return Err($cpu.raise_mem(exc::STORE_PROHIBITED, $addr));
        }
    };
}
...
pub(crate) fn exec_insn<B: Bus>(cpu: &mut Cpu, bus: &mut B, i: &Insn) -> Result<(), Trap> {
    ...
    macro_rules! cp0 {
        () => {
            if cpu.cpenable & 1 == 0 {
                return Err(cpu.raise(exc::COPROCESSOR0_DISABLED));
            }
        };
    }
    macro_rules! fr {
        ($n:expr) => {
            f32b(cpu.fr[$n as usize])
        };
    }
    macro_rules! setf {
        ($n:expr, $v:expr) => {{
            let v: f32 = $v;
            cpu.fr[$n as usize] = v.to_bits();
        }};
    }
    macro_rules! setb {
        ($n:expr, $v:expr) => {{
    ...
    match i.op {
        ...
        Simcall => {
            cpu.pc = next;
            return Err(Trap::Simcall);
        }
        Waiti => {
            cpu.ps = (cpu.ps & !ps::INTLEVEL_MASK) | (immu & 0xf);
            cpu.waiting = true;
        }
        Rsil => {
            let old = cpu.ps;
        ...
        Rfde => {
            new_pc = if cpu.excm() { cpu.depc } else { cpu.epc[1] };
            taken = true;
        }
        Rfi => {
            let l = (immu & 0xf) as usize;
            if !(2..=7).contains(&l) {
                return Err(cpu.raise(exc::ILLEGAL));
            }
            cpu.ps = cpu.eps[l];
            new_pc = cpu.epc[l];
            taken = true;
        ...
        Rfwo => {
            ...
            cpu.windowbase = (cpu.ps & ps::OWB_MASK) >> ps::OWB_SHIFT;
            cpu.ps &= !ps::EXCM;
            new_pc = cpu.epc[1];
            taken = true;
        }
        ...
        Retw | RetwN => {
            ...
            if cpu.windowstart & bit(newbase) != 0 {
                ...
                cpu.epc[1] = pc;
                cpu.ps |= ps::EXCM;
                cpu.pc = cpu.vecbase.wrapping_add(match n {
                    1 => vec::WINDOW_UF4,
                    2 => vec::WINDOW_UF8,
                    _ => vec::WINDOW_UF12,
                });
        ...
        Movsp => {
            let live = bit(cpu.windowbase.wrapping_sub(1))
                | bit(cpu.windowbase.wrapping_sub(2))
                | bit(cpu.windowbase.wrapping_sub(3));
            if cpu.windowstart & live == 0 {
                return Err(cpu.raise(exc::ALLOCA));
            }
            set!(t, ar!(s));
        }
        Rotw => cpu.rotate(imm),
        L32e => {
            ...
            set!(t, ld!(cpu, bus, read32, a));
        }
        S32e => {
            let a = ar!(s).wrapping_add(immu);
            st!(cpu, bus, write32, a, ar!(t));
        }
        S32nb => {
            let a = ar!(s).wrapping_add(immu);
            st!(cpu, bus, write32, a, ar!(t));
        ...
        L8ui => {
            let a = ar!(s).wrapping_add(immu);
            set!(t, ld!(cpu, bus, read8, a));
        }
        L16ui => {
            let a = ar!(s).wrapping_add(immu);
            set!(t, ld!(cpu, bus, read16, a));
        }
        L16si => {
            let a = ar!(s).wrapping_add(immu);
            set!(t, ld!(cpu, bus, read16, a) as u16 as i16 as i32 as u32);
        }
        L32i | L32iN | L32ai => {
            let a = ar!(s).wrapping_add(immu);
            set!(t, ld!(cpu, bus, read32, a));
        ...
        L32r => {
            set!(t, ld!(cpu, bus, read32, immu));
        }
        S8i => {
            let a = ar!(s).wrapping_add(immu);
            st!(cpu, bus, write8, a, ar!(t) as u8);
        }
        S16i => {
            let a = ar!(s).wrapping_add(immu);
            st!(cpu, bus, write16, a, ar!(t) as u16);
        }
        S32i | S32iN | S32ri => {
            let a = ar!(s).wrapping_add(immu);
            st!(cpu, bus, write32, a, ar!(t));
        ...
        Dpfr | Dpfw | Dpfro | Dpfwo | Dhwb | Dhwbi | Dhi | Dii | Ipf | Ihi | Iii | Ipfl | Ihu
        | Iiu | Dpfl | Dhu | Diu => {}
        ...
        Sext => {
            let b = imm as u32;
            let v = ar!(s);
            set!(r, (((v << (31 - b)) as i32) >> (31 - b)) as u32);
        }
        Clamps => {
            let b = imm;
            let v = ar!(s) as i32;
        ...
        Sll => {
            let sh = 32u32.wrapping_sub(cpu.sar) & 63;
            set!(r, if sh >= 32 { 0 } else { ar!(s) << sh });
        }
        Srl => set!(r, if cpu.sar >= 32 { 0 } else { ar!(t) >> cpu.sar }),
        Sra => set!(
            r,
            if cpu.sar >= 32 {
                ((ar!(t) as i32) >> 31) as u32
            } else {
                ((ar!(t) as i32) >> cpu.sar) as u32
            }
        ),
        Src => {
            let v = ((ar!(s) as u64) << 32) | ar!(t) as u64;
            set!(r, (v >> (cpu.sar & 63)) as u32);
        }
        ...
        Mulsh => set!(
            r,
            (((ar!(s) as i32 as i64) * (ar!(t) as i32 as i64)) >> 32) as u32
        ),
        Mul16u => set!(r, (ar!(s) & 0xffff).wrapping_mul(ar!(t) & 0xffff)),
        Mul16s => set!(
            r,
            ((ar!(s) as i16 as i32).wrapping_mul(ar!(t) as i16 as i32)) as u32
        ),
        Quou => {
            let d = ar!(t);
            if d == 0 {
                return Err(cpu.raise(exc::DIVIDE_BY_ZERO));
            }
            set!(r, ar!(s) / d);
        }
        Remu => {
        ...
        Quos => {
            ...
            set!(r, (ar!(s) as i32).wrapping_div(d) as u32);
        }
        Rems => {
            let d = ar!(t) as i32;
            if d == 0 {
            ...
            set!(r, (ar!(s) as i32).wrapping_rem(d) as u32);
        }

        // ------------------------------------------------------------ special / user registers
        Rsr => {
            let v = match cpu.read_sr(immu) {
                Some(v) => v,
                None => return Err(cpu.raise(exc::ILLEGAL)),
            };
            set!(t, v);
        }
        Wsr => {
            let v = ar!(t);
            if immu == sr::WINDOWBASE {
                cpu.windowbase = v & (NUM_WINDOWS - 1);
            }
            // takes effect immediately
            else if cpu.write_sr(immu, v).is_none() {
                return Err(cpu.raise(exc::ILLEGAL));
            }
        }
        Xsr => {
            let v = ar!(t);
            let old = match cpu.read_sr(immu) {
                Some(v) => v,
                None => return Err(cpu.raise(exc::ILLEGAL)),
            };
            cpu.write_sr(immu, v);
        ...
        Rur => {
            if immu == 232 || immu == 233 {
                cp0!();
            }
            let v = match cpu.read_ur(immu) {
                Some(v) => v,
                None => return Err(cpu.raise(exc::ILLEGAL)),
            };
            set!(r, v);
        ...
        Wur => {
            ...
            let v = ar!(t);
            if cpu.write_ur(immu, v).is_none() {
                return Err(cpu.raise(exc::ILLEGAL));
            }
        }
        Rer => set!(t, 0),
        ...
        Ssi | Ssip => {
            cp0!();
            let a = ar!(s).wrapping_add(immu);
            st!(cpu, bus, write32, a, cpu.fr[t as usize]);
            if i.op == Ssip {
                set!(s, a);
            }
        }
        ...
        Ssx | Ssxp => {
            cp0!();
            let a = ar!(s).wrapping_add(ar!(t));
            st!(cpu, bus, write32, a, cpu.fr[r as usize]);
            if i.op == Ssxp {
                set!(s, a);
            }
        }
        ...
        RoundS => {
            cp0!();
            let v = fr!(s) * (1u64 << imm) as f32;
            set!(r, sat_i32(v.round_ties_even()));
        }
        TruncS => {
            cp0!();
            let v = fr!(s) * (1u64 << imm) as f32;
            set!(r, sat_i32(v.trunc()));
        }
        FloorS => {
            cp0!();
            let v = fr!(s) * (1u64 << imm) as f32;
            set!(r, sat_i32(v.floor()));
        }
        CeilS => {
            cp0!();
            let v = fr!(s) * (1u64 << imm) as f32;
            set!(r, sat_i32(v.ceil()));
        }
        UtruncS => {
            cp0!();
            let v = fr!(s) * (1u64 << imm) as f32;
            set!(r, sat_u32(v.trunc()));
        }
        FloatS => {
            cp0!();
            setf!(r, (ar!(s) as i32 as f32) / (1u64 << imm) as f32);
        }
        UfloatS => {
            cp0!();
            setf!(r, (ar!(s) as f32) / (1u64 << imm) as f32);
        }
        MovS => {
            cp0!();
            cpu.fr[r as usize] = cpu.fr[s as usize];
        }
        AbsS => {
            cp0!();
            cpu.fr[r as usize] = cpu.fr[s as usize] & 0x7fff_ffff;
        }
        NegS => {
            cp0!();
            cpu.fr[r as usize] = cpu.fr[s as usize] ^ 0x8000_0000;
        }
        Rfr => {
            cp0!();
            set!(r, cpu.fr[s as usize]);
        }
        Wfr => {
            cp0!();
            cpu.fr[r as usize] = ar!(s);
        }
        ConstS => {
            cp0!();
            setf!(
        ...
        Div0S | Nexp01S | Recip0S | Rsqrt0S | Sqrt0S | AddexpS => {
            cp0!();
        }
        MkdadjS => {
            cp0!();
        ...
        UnS => {
            cp0!();
            setb!(r, fr!(s).is_nan() || fr!(t).is_nan());
        }
        OeqS => {
            cp0!();
            setb!(r, fr!(s) == fr!(t));
        ...
        UeqS => {
            cp0!();
            setb!(r, fr!(s) == fr!(t) || fr!(s).is_nan() || fr!(t).is_nan());
        }
        OltS => {
            cp0!();
            setb!(r, fr!(s) < fr!(t));
...
fn exec_mac16<B: Bus>(cpu: &mut Cpu, bus: &mut B, i: &Insn) -> Result<(), Trap> {
    ...
    let set_acc = |cpu: &mut Cpu, v: i64| {
        cpu.acclo = v as u32;
        cpu.acchi = ((v >> 32) as u32) & 0xff;
    };
    ...
    let (x, y) = match op2 {
        ...
        8 | 9 => {
            // LDINC / LDDEC mw, as
            let a = if op2 == 8 {
                cpu.get_ar(s).wrapping_add(4)
            } else {
                cpu.get_ar(s).wrapping_sub(4)
            };
            let v = match bus.read32(a) {
                Ok(v) => v,
                Err(_) => return Err(cpu.raise_mem(exc::LOAD_PROHIBITED, a)),
            };
    ...
    let xh = half & 2 != 0;
    let yh = half & 1 != 0;
    let kind_eff = if op2 == 0 || op2 == 1 || op2 == 4 || op2 == 5 {
        2
    } else {
        kind
    }; // *.ldinc/lddec forms are always MULA
    let prod = if kind_eff == 0 {
        uhl(x, xh) * uhl(y, yh)
    } else {
        hl(x, xh) * hl(y, yh)
    };
    let v = match kind_eff {
        0 | 1 => prod,
        2 => acc(cpu) + prod,
        _ => acc(cpu) - prod,
    };
    set_acc(cpu, v);
    if op2 == 0 || op2 == 1 || op2 == 4 || op2 == 5 {
        // with LDINC/LDDEC of MR[r&3]? (mw = r field) — load after multiply
        let a = if op2 == 0 || op2 == 4 {
            cpu.get_ar(s).wrapping_add(4)
        } else {
            cpu.get_ar(s).wrapping_sub(4)
        };
        let ld = match bus.read32(a) {
            Ok(v) => v,
            Err(_) => return Err(cpu.raise_mem(exc::LOAD_PROHIBITED, a)),
        };
