...
pub struct BlockInsn {
    pub insn: Insn,
    pub max_ar: u8,
    /// byte offset of this instruction's native code from the block body start
    pub off: u32,
}
...
#[cfg(target_arch = "wasm32")]
const ARENA_MAX: usize = 1 << 17; // 4 MB per core in the browser
...
impl BlockCache {
    pub fn new() -> Self {
        BlockCache {
            ...
            arena: Vec::with_capacity(ARENA_MAX + MAX_LEN),
            resume: (0, 0, 1),
            builds: 0,
            flushes: 0,
            code: crate::jit::CodeCache::new(CODE_SIZE),
            helpers: None,
            jit_enabled: crate::jit::AVAILABLE,
            compiled: 0,
        }
    }
    ...
    pub fn code_bytes(&self) -> usize {
        self.code.as_ref().map(|c| c.used()).unwrap_or(0)
    }
    pub fn jit_active(&self) -> bool {
        self.jit_enabled && self.code.is_some()
    }
    #[inline(always)]
    fn index(pc: u32) -> usize {
        ((pc >> 1) ^ (pc >> 16)) as usize & (ENTRIES - 1)
    }
    #[inline(always)]
    fn valid(e: &Entry, pv: &[u32]) -> bool {
        pv.get(e.vidx[0] as usize).copied().unwrap_or(0) == e.ver[0]
            && pv.get(e.vidx[1] as usize).copied().unwrap_or(0) == e.ver[1]
    }
}
...
impl Clone for BlockCache {
    fn clone(&self) -> Self {
        let mut b = Self::new();
        b.jit_enabled = self.jit_enabled;
        b
    }
}
...
pub(crate) fn ends_block(i: &Insn) -> bool {
    ...
    match i.op {
        ...
        | Dsync | Excw | J | Jx | Call0 | Call4 | Call8 | Call12 | Callx0 | Callx4 | Callx8
        | Callx12 | Ret | RetN | Retw | RetwN | Rotw | Rfe | Rfue | Rfde | Rfwo | Rfwu | Rfi
        | Rfme | Beqz | Bnez | Bltz | Bgez | BeqzN | BnezN | Beqi | Bnei | Blti | Bgei | Bltui
        | Bgeui | Bnone | Beq | Blt | Bltu | Ball | Bbc | Bbci | Bany | Bne | Bge | Bgeu
        | Bnall | Bbs | Bbsi | Bf | Bt | Loop | Loopnez | Loopgtz | Wsr | Xsr => true,
        _ => i.len == 0,
    }
}
...
pub(crate) fn must_start_block(i: &Insn) -> bool {
    matches!(i.op, Op::Rsr | Op::Wsr | Op::Xsr)
        && matches!(
            i.imm as u32,
            sr::CCOUNT | sr::INTERRUPT | sr::INTCLEAR | sr::INTENABLE | sr::PS | sr::ICOUNT | 240
                ..=242
        )
}
...
fn build<B: Bus>(cpu: &mut Cpu, bus: &mut B, pc0: u32) -> Result<(u32, u32, u16), Trap> {
    ...
    loop {
        let bytes = match bus.fetch(pc) {
            ...
            Err(_) => {
                if n == 0 {
                    return Err(cpu.raise_mem(crate::state::exc::IFETCH_ERROR, pc));
                }
                break;
            }
        };
        let i = decode(pc, bytes);
        if n > 0 && (must_start_block(&i) || cpu.boundary_bloom & pc_bit(pc) != 0) {
            break;
        }
        cpu.blocks.arena.push(BlockInsn {
            insn: i,
            max_ar: max_ar(&i),
            off: 0,
        });
        ...
        if ends_block(&i) || n as usize == MAX_LEN {
            break;
        }
    }
    let last_byte = last.wrapping_add(
        cpu.blocks.arena[(start + n as u32 - 1) as usize]
            .insn
            .len
            .max(1) as u32
            - 1,
    ...
    let vidx1 = if last_byte >> 7 != pc0 >> 7 {
        bus.code_page(last_byte)
    } else {
        vidx0
    }; // pages are >= 128 B
    let pv = bus.page_versions();
    let ver = [
        pv.get(vidx0 as usize).copied().unwrap_or(0),
        pv.get(vidx1 as usize).copied().unwrap_or(0),
    ];
...
pub fn run_block<B: Bus>(cpu: &mut Cpu, bus: &mut B, budget: u32) -> (u32, Option<Trap>) {
    ...
    let (ei, mut k, end) = {
        ...
        if rpc == pc
            && e.pc != 1
            && BlockCache::valid(&e, bus.page_versions())
            && rk >= e.start
            && rk < e.start + e.n as u32
        {
            (rei, rk, e.start + e.n as u32)
        } else {
            let ei = BlockCache::index(pc);
            let e = cpu.blocks.entries[ei];
            if e.pc == pc && BlockCache::valid(&e, bus.page_versions()) {
                (ei as u32, e.start, e.start + e.n as u32)
            } else {
                match build(cpu, bus, pc) {
                    Ok((ei, s, n)) => (ei, s, s + n as u32),
                    Err(t) => return (1, Some(t)),
                }
            }
        }
    };
    ...
    while done < limit {
        ...
        if let Some(t) = cpu.check_overflow(e.max_ar) {
            trap = Some(t);
            pre = true;
            break;
        }
        ...
        if cpu.pc != expected || bus.block_break() {
            broke = true;
            break;
        }
    }
    ...
    if trap.is_none() && !broke && k < end {
        cpu.blocks.resume = (ei, k, cpu.pc);
    }
