...
pub fn exec<B: Bus>(cpu: &mut Cpu, bus: &mut B, i: &Insn) -> Result<(), Trap> {
    ...
    match p.kind {
        ...
        Kind::LdQacc { h, high32 } => { let a = ar!(As); if high32 { let v = ld(cpu, bus, a, 4)? as u32; if h { cpu.qacc_h[4] = v; } else { cpu.qacc_l[4] = v; } } else { let v = ld(cpu, bus, a, 16)?; let arr = if h { &mut cpu.qacc_h } else { &mut cpu.qacc_l }; for k in 0..4 { arr[k] = (v >> (32 * k)) as u32; } } post!(Mode::Ip); }
        ...
        Kind::Ldf { n, mode } => { let v = ld(cpu, bus, ar!(As), 4 * n as u32)?; let roles = [Fu0, Fu1, Fu2, Fu3]; for k in 0..n as usize { let r = o.get(roles[k]) as usize & 15; cpu.fr[r] = (v >> (32 * k)) as u32; } post!(mode); }
        Kind::Stf { n, mode } => { let roles = [Fv0, Fv1, Fv2, Fv3]; let mut v = 0u128; for k in 0..n as usize { v |= (cpu.fr[o.get(roles[k]) as usize & 15] as u128) << (32 * k); } st(cpu, bus, ar!(As), 4 * n as u32, v)?; post!(mode); }
