...
pub struct ClockTree<const N: usize> {
    pub cpu_hz: u64,
    cycle_total: u64,
    done: [u64; N],
}
...
impl<const N: usize> ClockTree<N> {
    ...
    pub fn advance(
        ...
        divs: &Dividers<N>,
        cycles: u64,
        mut f: impl FnMut(ClockDomain, u64),
    ) {
    ...
    pub const fn divider(divs: &Dividers<N>, d: ClockDomain) -> u64 {
        divider(divs, d)
    }
}
...
mod tests {
    ...
    fn delivered_ticks_never_drift() {
        ...
        for c in [1u64, 7, 64, 512, 5, 3, 100] {
            t.advance(&D, c, |d, n| match d {
                ClockDomain::Systimer => st += n,
                ClockDomain::Apb => apb += n,
                _ => {}
            });
        }
        assert_eq!(st, 692 / 15);
        assert_eq!(apb, 692 / 3);
        assert_eq!(t.cycles(), 692);
        assert_eq!(ClockTree::<2>::divider(&D, ClockDomain::Apb), 3);
        assert_eq!(ClockTree::<2>::divider(&D, ClockDomain::Cpu), 1);
    }
}
