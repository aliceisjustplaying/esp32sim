...
async function runManifest(name) {
  ...
  if (w.esp32sim_profile_report && process.env.CENSUS_OUT) { const n0 = logs.length; w.esp32sim_profile_report(emu); writeFileSync(process.env.CENSUS_OUT, logs.slice(n0).join('\n') + `\n[census-total] insns=${insns} jit_insns=${w.esp32sim_block_jit_insns(emu)} jit_stats=${JSON.stringify(blockJit.stats)}\n`); }
  w.esp32sim_delete(emu);
  const wall = (Date.now() - t0) / 1000;
...
if (!process.env.CENSUS_OUT) try { await testJitHandoff(); } catch (e) { failures++; console.error(`FAIL wasm JIT handoff: ${e.message}`); }
