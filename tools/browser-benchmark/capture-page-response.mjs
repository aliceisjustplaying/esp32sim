...
const [url, output, port = '9239'] = process.argv.slice(2);
if (!url || !output) throw Error('Usage: capture-page-response.mjs URL OUTPUT_DIRECTORY [HEADLESS_DEBUG_PORT]');
...
socket.onclose = () => { for (const task of pending.values()) task.reject(Error('Browser disconnected')); pending.clear(); };
...
try {
  ...
  const mouse = (type, x, y) => send('Input.dispatchMouseEvent', {type,x:rect.x+x*rect.sx,y:rect.y+y*rect.sy,
    button:type==='mouseMoved'?'none':'left',buttons:type==='mouseReleased'?0:1,clickCount:type==='mouseMoved'?0:1}, sessionId);
