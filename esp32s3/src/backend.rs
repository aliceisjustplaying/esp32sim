...
impl MeasuredBootScheduler {
    ...
    fn advance_waiting_core(&mut self, core: CoreId) -> Result<bool, MeasuredStepError> {
        ...
        let cpu = match core {
            CoreId::Core0 => &self.machine.cpu,
            CoreId::Core1 => &self.machine.cpu1,
        };
        ...
        self.machine.advance_measured_clocks(core, elapsed);
    ...
    fn console_contains(&self, marker: &[u8]) -> bool {
        !marker.is_empty()
            && [
                self.machine.console.as_slice(),
                self.machine.console_usb.as_slice(),
                self.machine.console_uart0.as_slice(),
    ...
    fn refusal(&self, core: CoreId, error: MeasuredStepError) -> MeasuredBootStop {
        let index = core_index(core);
        let pc = match core {
            CoreId::Core0 => self.machine.cpu.pc,
            CoreId::Core1 => self.machine.cpu1.pc,
        };
...
mod tests {
    ...
    impl backend_api::DeadlineModel for FailingDeadlineBoard {
        ...
        fn advance_to(
            ...
            Err(backend_api::DeadlineError::TimeReversed {
                current: cycle + 1,
                requested: cycle,
            })
        }
    ...
    fn reset_configuration_uses_the_xtal_clock_and_register_defaults() {
        let machine = crate::Machine::new([0; 6]);
        assert_eq!(
            chip_config_from_registers(&machine),
            ChipConfig {
                cpu_mhz: 40,
    ...
    fn waiting_core_deadline_failure_stops_with_typed_refusal() {
        ...
        assert!(matches!(
            ...
            MeasuredBootStop::Refusal {
                ...
                core_cycles: [7, 0],
                refusal: MeasuredBootRefusal {
                    error: MeasuredStepError::Deadline(backend_api::DeadlineError::TimeReversed {
                        current: 8,
                        requested: 7,
                    }),
                    core: CoreId::Core0,
