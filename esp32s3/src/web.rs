...
impl WebServer {
    ...
    pub fn send_text(&self, s: &str) {
        self.emit(1, s.as_bytes());
    }
    pub fn send_binary(&self, d: &[u8]) {
        self.emit(2, d);
    }
    fn emit(&self, kind: u8, d: &[u8]) {
        let queue = self
            .shared
            .lock()
            .expect("web server shared state mutex poisoned")
            .queue;
        if queue {
            self.shared
                .lock()
...
fn handle_client(mut stream: TcpStream, shared: Arc<Mutex<Shared>>) {
    ...
    let Some(key) = key else {
        ...
        let (status, body, ctype) = match body {
            Some(b) => (
                "200 OK",
                b,
    ...
    let _ = stream.set_nodelay(true);
    {
        ...
        let mut out = stream
            .try_clone()
            .expect("connected client stream should be clonable");
        std::thread::spawn(move || {
        ...
        sh.clients.push(Client {
            tx,
            peer: stream.peer_addr().ok(),
        });
    }
    // read loop
    let mut read_exact = |n: usize| -> Option<Vec<u8>> {
        let mut v = vec![0u8; n];
        let mut got = 0;
        while got < n {
            match stream.read(&mut v[got..]) {
    ...
    while let Some(h) = read_exact(2) {
        ...
        if len == 126 {
            let Some(e) = read_exact(2) else { break };
            len = u16::from_be_bytes([e[0], e[1]]) as u64;
        } else if len == 127 {
            let Some(e) = read_exact(8) else { break };
            len = u64::from_be_bytes(
                e.try_into()
                    .expect("eight-byte WebSocket length should convert to an array"),
            );
        }
        let mask = if masked {
        ...
        let Some(mut data) = read_exact(len as usize) else {
            break;
        };
        if masked {
