...
impl VirtualNet {
    pub fn new(log: bool) -> Self {
        VirtualNet {
            gw_mac: [0x02, 0x53, 0x49, 0x4d, 0x00, 0x02],
            gw_ip: [10, 0, 2, 2],
            dns_ip: [10, 0, 2, 3],
            sta_ip: [10, 0, 2, 15],
            mask: [255, 255, 255, 0],
    ...
    pub fn handle(&mut self, eth: &[u8], now_us: u64) -> Vec<Vec<u8>> {
        ...
        match be16(&eth[12..14]) {
            ...
            et => {
                self.unhandled += 1;
                if self.log {
                    eprintln!("[net] ignoring ethertype {:#06x} ({} bytes)", et, eth.len());
                }
                Vec::new()
            }
        }
    ...
    fn frame(&self, dst: &[u8; 6], ethertype: u16, payload: &[u8]) -> Vec<u8> {
        ...
        f.extend_from_slice(&self.gw_mac);
    ...
    fn arp(&mut self, p: &[u8], src: &[u8; 6]) -> Vec<Vec<u8>> {
        if p.len() < 28 || be16(&p[6..8]) != 1 {
            return Vec::new();
        } // only requests
        let target = &p[24..28];
        if target == self.sta_ip {
            return Vec::new();
        } // not ours to answer
        let mut r = Vec::with_capacity(28);
        r.extend_from_slice(&[0, 1, 8, 0, 6, 4, 0, 2]); // ethernet/ipv4, reply
        r.extend_from_slice(&self.gw_mac);
        r.extend_from_slice(target); // sender = the address asked for
        r.extend_from_slice(src);
        r.extend_from_slice(&p[14..18]); // target = the station
        self.arp_replies += 1;
        if self.log {
            eprintln!(
                "[net] ARP who-has {}.{}.{}.{} -> {}",
                ...
                target[2],
                target[3],
                crate::wifi::mac_str(&self.gw_mac)
            );
        }
    ...
    fn ipv4(&mut self, p: &[u8], src: &[u8; 6]) -> Vec<Vec<u8>> {
        ...
        let mut sip = [0u8; 4];
        sip.copy_from_slice(&p[12..16]);
        let mut dip = [0u8; 4];
        dip.copy_from_slice(&p[16..20]);
        match proto {
            ...
            17 if self.nat.is_some() => {
                ...
                let nat = self
                    .nat
                    .as_mut()
                    .expect("the NAT UDP branch requires a network backend");
            ...
            6 if self.nat.is_some() => {
                ...
                self.nat
                    .as_mut()
                    .expect("the NAT TCP branch requires a network backend")
                    .tcp_in(src, &sip, &dip, body, now)
            }
            17 if body.len() >= 8 && be16(&body[2..4]) == 53 => {
                self.dns(&body[8..], src, &sip, &dip, be16(&body[0..2]))
            }
            17 if body.len() >= 8 && be16(&body[2..4]) == 123 => {
                self.ntp(&body[8..], src, &sip, &dip, be16(&body[0..2]))
            }
            6 if body.len() >= 20 && body[13] & 0x02 != 0 && body[13] & 0x10 == 0 => {
                self.tcp_reject(body, src, &sip, &dip)
            }
            1 if !body.is_empty() && body[0] == 8 => {
                // ICMP echo request
                let mut icmp = body.to_vec();
                icmp[0] = 0;
                icmp[2] = 0;
                icmp[3] = 0;
                let c = checksum(&icmp, 0).to_be_bytes();
                icmp[2] = c[0];
                icmp[3] = c[1];
                self.pings += 1;
                let mut dst_ip = [0u8; 4];
                dst_ip.copy_from_slice(&p[12..16]);
                let mut src_ip = [0u8; 4];
                src_ip.copy_from_slice(&p[16..20]);
                if self.log {
                    eprintln!("[net] ICMP echo request -> reply ({} bytes)", icmp.len());
                }
            ...
            _ => {
                self.unhandled += 1;
                if self.log {
                    eprintln!("[net] ignoring IPv4 proto {} ({} bytes)", proto, p.len());
                }
                Vec::new()
            }
        }
    ...
    fn dhcp(&mut self, d: &[u8], src: &[u8; 6]) -> Vec<Vec<u8>> {
        ...
        b[0] = 2;
        b[1] = 1;
        b[2] = 6; // BOOTREPLY, ethernet, 6-byte MAC
        ...
        b.extend_from_slice(&self.gw_ip); // server id
        b.extend_from_slice(&[51, 4, 0, 1, 0x51, 0x80]); // lease 86400 s
        b.extend_from_slice(&[1, 4]);
        b.extend_from_slice(&self.mask);
        b.extend_from_slice(&[3, 4]);
        b.extend_from_slice(&self.gw_ip); // router
        b.extend_from_slice(&[6, 4]);
        b.extend_from_slice(&self.dns_ip); // DNS
        b.push(255);
        while b.len() < 300 {
            b.push(0);
        }
        ...
        let (l2, l3) = if want_bcast {
            ([0xffu8; 6], [255u8, 255, 255, 255])
        } else {
            (*src, self.sta_ip)
        };
...
impl VirtualNet {
    ...
    fn dns(
        ...
        if q.len() < 12 || be16(&q[2..4]) & 0x8000 != 0 {
            return Vec::new();
        }
        let mut i = 12;
        ...
        if self.log {
            eprintln!(
                ...
                if qtype == 1 {
                    format!(
                        "{}.{}.{}.{}",
                        self.dns_ip[0], self.dns_ip[1], self.dns_ip[2], self.dns_ip[3]
                    )
                } else {
                    "(none)".into()
                }
    ...
    #[expect(
        clippy::precedence,
        clippy::identity_op,
        reason = "the existing NTP fractional calculation must remain unchanged"
    )]
    fn ntp(
        &mut self,
        q: &[u8],
        ...
        self.ntp_answers += 1;
        if self.log {
            eprintln!("[net] NTP request -> host time ({} s since 1900)", secs);
        }
    ...
    fn tcp_reject(
        ...
        pseudo.extend_from_slice(dip);
        pseudo.extend_from_slice(sip);
        pseudo.extend_from_slice(&[0, 6, 0, 20]);
        pseudo.extend_from_slice(&r);
        let c = checksum(&pseudo, 0).to_be_bytes();
        r[16] = c[0];
        r[17] = c[1];
        self.tcp_rejects += 1;
        if self.log {
            eprintln!(
                "[net] TCP {}.{}.{}.{}:{} -> refused (no NAT backend yet)",
...
fn ip_packet(proto: u8, src: &[u8; 4], dst: &[u8; 4], payload: &[u8]) -> Vec<u8> {
    ...
    let c = checksum(&h, 0).to_be_bytes();
    h[10] = c[0];
    h[11] = c[1];
