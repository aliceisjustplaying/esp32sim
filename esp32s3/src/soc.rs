...
use crate::bus::{
    SocBus, DBUS_HIGH, DBUS_LOW, IBUS_HIGH, IBUS_LOW, MMU_ENTRIES, MMU_INVALID, SRC_FLASH,
};
...
pub fn machine(mac: [u8; 6]) -> Machine {
    let mut m = Machine::new(mac, SocBus::new(8 << 20, 2 << 20, mac));
...
impl Soc for S3 {
    ...
    fn new_core(i: usize) -> Cpu {
        Cpu::new(if i == 0 { 0xCDCD } else { 0xABAB })
    }
...
impl esp_soc::SocBus for SocBus {
    ...
    fn sw_reset(&self) -> bool {
        self.periph.rtc.sw_reset
    }
    fn reset_cause(&self) -> u32 {
        self.periph.rtc.reset_cause
    }
    ...
    fn console_take(&mut self) -> [Vec<u8>; 4] {
        [
            std::mem::take(&mut self.periph.usb.tx_out),
    ...
    fn set_flash_size(&mut self, bytes: usize) {
        ...
        self.periph.spi1.jedec[2] = cap;
        self.periph.spi0.jedec[2] = cap;
    ...
    fn set_strap(&mut self, v: u32) {
        self.periph.gpio.strap = v;
    }
    fn set_reset_cause(&mut self, c: u32) {
        self.periph.rtc.ram.write(0x38, c | (c << 6));
    }
    fn report(&self) -> String {
        ...
        if p.lcd_cam.lcd_frames > 0 {
            s += &format!("[emu] lcd: {} RGB frames\n", p.lcd_cam.lcd_frames);
        }
        if p.lcd_cam.frames + p.lcd_cam.dropped > 0 {
            s += &format!(
    ...
    fn observe_gpio(&mut self, on: bool) {
        self.gpio_events = if on { Some(Vec::new()) } else { None };
    }
    ...
    fn audio(&self) -> (&[i16], u32) {
        let a = self.periph.audio();
        (&a.pcm, a.sample_rate)
    }
    fn camera_frames(&self) -> u64 {
        self.periph.lcd_cam.frames
    }
    fn irq_sources_of(&self, core: usize, line: u32) -> Vec<usize> {
        (0..NUM_SOURCES)
            .filter(|&s| self.periph.intmatrix.map[core][s] == line)
            .collect()
    }
}
