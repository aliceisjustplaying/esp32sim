...
impl Nat {
    ...
    pub fn udp_out(
        ...
        let idx = self
            .udp
            .iter()
            .position(|f| f.guest_port == sport && f.dst_ip == *dip && f.dst_port == dport);
        let idx = match idx {
            Some(i) => i,
            None => {
        ...
        let _ = f
            .sock
            .send_to(payload, SocketAddr::new(IpAddr::V4(ip(dip)), dport));
    ...
    pub fn tcp_in(
        ...
        let flags = seg[13];
        let data = if seg.len() > off {
            &seg[off..]
        } else {
            &[][..]
        };
        let (syn, fin, rst, is_ack) = (
            flags & 2 != 0,
            flags & 1 != 0,
            flags & 4 != 0,
            flags & 0x10 != 0,
        ...
        let idx = self
            .tcp
            .iter()
            .position(|c| c.guest_port == sport && c.dst_port == dport && c.dst_ip == *dip);
        ...
        if is_ack {
            // release acknowledged bytes
            let c = &mut self.tcp[i];
            let acked = ack.wrapping_sub(c.unacked_seq) as usize;
            if acked > 0 && acked <= c.unacked.len() {
                c.unacked.drain(..acked);
                c.unacked_seq = ack;
            }
        }
        if !data.is_empty() {
        ...
        if fin {
            ...
            c.state = if c.state == State::HostClosed {
                State::Done
            } else {
                State::GuestClosed
            };
    ...
    pub fn poll(&mut self, now_us: u64) -> Vec<Vec<u8>> {
            ...
            if matches!(self.tcp[i].state, State::Established | State::GuestClosed) {
                ...
                loop {
                    let n = match self.tcp[i].sock.as_mut().map(|s| s.read(&mut buf)) {
                        Some(Ok(0)) => {
                            break_eof(&mut self.tcp[i]);
                            out.push(self.segment(i, 0x11, &[]));
                            self.tcp[i].our_seq = self.tcp[i].our_seq.wrapping_add(1);
                            break;
                        }
                        Some(Ok(n)) => n,
            ...
            loop {
                match self.udp[i].sock.recv_from(&mut buf) {
                    Ok((n, _from)) => {
                        if self.log {
                            eprintln!(
                                "[nat] UDP reply {} bytes -> guest port {}",
                                n, self.udp[i].guest_port
                            );
                        }
                        self.bytes_to_guest += n as u64;
        ...
        self.tcp.retain(|c| {
            c.state != State::Done && now_us.wrapping_sub(c.last_activity_us) < IDLE_CLOSE_US
        });
        self.udp
            .retain(|f| now_us.wrapping_sub(f.last_activity_us) < IDLE_CLOSE_US);
        out
    }

    fn unacked_full(&self, i: usize) -> bool {
        self.tcp[i].unacked.len() >= WINDOW as usize
    }
    ...
    fn segment_at(&self, i: usize, flags: u8, payload: &[u8], seq: u32) -> Vec<u8> {
        ...
        t.extend_from_slice(&c.dst_port.to_be_bytes());
        t.extend_from_slice(&c.guest_port.to_be_bytes());
        t.extend_from_slice(&seq.to_be_bytes());
        t.extend_from_slice(&c.guest_seq.to_be_bytes());
...
fn break_eof(c: &mut Tcp) {
    c.state = if c.state == State::GuestClosed {
        State::Done
    } else {
        State::HostClosed
    };
...
fn tcp_checksum(src: &[u8; 4], dst: &[u8; 4], seg: &[u8]) -> u16 {
    ...
    p.extend_from_slice(&(seg.len() as u16).to_be_bytes());
...
fn eth_ip(gmac: &[u8; 6], src: &[u8; 4], dst: &[u8; 4], proto: u8, payload: &[u8]) -> Vec<u8> {
    ...
    f.extend_from_slice(gmac);
    f.extend_from_slice(&[0x02, 0x53, 0x49, 0x4d, 0x00, 0x02]);
