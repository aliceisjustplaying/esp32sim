...
/// A transaction waiting for the board attached to a GP-SPI host. `rx_word_base` is controller
/// state, not part of the board contract: it selects the W-register half that receives MISO.
pub(crate) struct GpSpiTransfer { pub tx: Vec<u8>, pub rx_len: usize, rx_word_base: usize }
...
/// USR until the transfer is done. Transfers complete synchronously through the board model so a
/// command followed immediately by a result read sees the device response.
pub struct GpSpi { pub regs: RegRam, pub w: [u32; 16], pub int_raw: u32, pub int_ena: u32, pending: Option<GpSpiTransfer>, pub transfers: u64, pub log: bool }
impl GpSpi {
    pub fn new() -> Self { GpSpi { regs: RegRam::new(), w: [0; 16], int_raw: 0, int_ena: 0, pending: None, transfers: 0, log: std::env::var("ESP_EMU_DEBUG_SPI2").is_ok() } }
    pub fn irq(&self) -> bool { self.int_raw & self.int_ena != 0 }
    pub fn read(&self, off: u32) -> u32 {
        match off {
    ...
    fn transfer(&mut self) {
        ...
        let rx_len = if user & (1 << 28) != 0 {
            (((self.regs.read(0x20) & 0x3ffff) + 1 + 7) / 8) as usize
        } else {
            0
        };
        let rx_word_base = if user & (1 << 24) != 0 { 8 } else { 0 };
        self.pending = Some(GpSpiTransfer { tx, rx_len, rx_word_base });
    }
    pub(crate) fn take_transfer(&mut self) -> Option<GpSpiTransfer> { self.pending.take() }
    pub(crate) fn finish_transfer(&mut self, transfer: GpSpiTransfer, rx: &[u8]) {
        if transfer.rx_len != 0 {
            ...
            for i in 0..transfer.rx_len.min(capacity) {
                ...
                let word = transfer.rx_word_base + i / 4; let shift = 8 * (i % 4);
                self.w[word] = (self.w[word] & !(0xff << shift)) | ((b as u32) << shift);
            }
        }
        if self.log { eprintln!("[spi2] transfer tx={} rx={}: {:02x?}", transfer.tx.len(), transfer.rx_len, &transfer.tx[..16.min(transfer.tx.len())]); }
        self.transfers += 1;
...
impl I2s {
    pub fn new() -> Self { I2s { rx_conf: 0, tx_conf: 0, int_raw: 0, int_ena: 0, ram: RegRam::new(), tx_conf1: 0, tx_clkm_conf: 0, tx_clkm_div_conf: 0, tx_tdm_ctrl: 0, sample_rate: 44100, bytes_per_frame: 4, acc: 0, pcm: Vec::new(), frames_out: 0, tx_started_log: false } }
    ...
    pub fn derive_rate(&self) -> Option<u32> {
        ...
        let src: u64 = match (c >> 27) & 3 { 0 => 40_000_000, 3 => return None, _ => 160_000_000 };   // XTAL / PLL_F160M / external
        let n = (c & 0xff) as u64;
        if n == 0 { return None; }
