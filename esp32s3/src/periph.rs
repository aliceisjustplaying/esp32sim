...
pub use esp_periph::{
    read_desc, reset_cause_name, Aes, DirtyMem, DmaDesc, Efuse, Gdma, GdmaInCh, GdmaOutCh, Gpio,
    I2s, RegRam, Rmt, RmtTxCh, Rsa, RtcCntl, Sha, SpiMem, SystemRegs, Systimer, Timer, TimerGroup,
    Uart, UsbSerialJtag, APB_HZ, DMA_ADDR_BASE, GDMA_CHANNELS, GDMA_CH_STRIDE, RMT_MEM_WORDS,
    RST_POWERON, RST_RTCWDT_CPU, RST_RTCWDT_RTC, RST_RTCWDT_SYS, RST_SW_CPU, RST_SW_SYS,
    RTC_SLOW_HZ, SYSTIMER_HZ, XTAL_HZ,
};
...
impl Device for IntMatrix {
    fn read(&mut self, off: u32) -> u32 {
        IntMatrix::read(self, off)
    }
    fn write(&mut self, off: u32, v: u32) -> WriteEffect {
        IntMatrix::write(self, off, v);
        WriteEffect::INTMAP
    }
}
...
impl Device for LcdCam {
    fn read(&mut self, off: u32) -> u32 {
        LcdCam::read(self, off)
    }
    fn write(&mut self, off: u32, v: u32) -> WriteEffect {
        LcdCam::write(self, off, v);
        WriteEffect::NONE
    }
    fn irq_sources(&self) -> u64 {
        self.irq() as u64
...
impl Device for FeIq {
    fn read(&mut self, _off: u32) -> u32 {
        self.word | if self.done { 1 << 16 } else { 0 }
    }
...
impl Peripherals {
    ...
    pub fn write32(&mut self, addr: u32, v: u32) {
        ...
        if fx.contains(WriteEffect::SPI_EXEC) {
            self.spi_exec = true;
        }
        if fx.contains(WriteEffect::INTMAP) {
