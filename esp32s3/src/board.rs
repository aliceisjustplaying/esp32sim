...
impl BoardModel for WaveshareAmoled18V2 {
    ...
    fn schedule_spi2_dma(
        ...
        if request.clock_hz != 40_000_000 {
            return Err(Spi2DmaTimingRefusal::UnsupportedClock { clock_hz: request.clock_hz });
        }
    ...
    fn next_deadline(&self) -> Option<VirtualCycle> {
        let touch_or_te = self
            .pending_touch_irq
            .map_or(self.next_te_cycle, |(cycle, _)| cycle.min(self.next_te_cycle));
        Some(
...
mod co5300_tests {
    ...
    fn unreceipted_spi2_dma_clock_and_mode_refuse_by_name() {
        ...
        assert_eq!(
            ...
            Err(Spi2DmaTimingRefusal::UnsupportedClock { clock_hz: 20_000_000 })
        );
        assert_eq!(
            board.schedule_spi2_dma(Spi2DmaRequest {
                submitted_at: 0,
            ...
            Err(Spi2DmaTimingRefusal::UnsupportedMode { mode: Spi2Mode::Single })
        );
    }
    #[test]
