...
pub trait BoardModel {
    ...
    fn input_changes(&mut self, _cycles: u64) -> Vec<(u8, bool)> {
        Vec::new()
    }
}
...
impl BoardModel for WaveshareAmoled18V2 {
    ...
    fn input_changes(&mut self, cycles: u64) -> Vec<(u8, bool)> {
        ...
        while self.te_acc >= half_period {
            self.te_acc -= half_period;
            self.te_level = !self.te_level;
            changes.push((13, self.te_level));
        }
        changes
    }
}
...
mod co5300_tests {
    ...
    fn amoled_board_generates_sixty_hz_tear_signal() {
        ...
        assert_eq!(
            board.input_changes(crate::periph::CPU_HZ / 120),
            [(13, false)]
        );
        assert_eq!(
            board.input_changes(crate::periph::CPU_HZ / 120),
