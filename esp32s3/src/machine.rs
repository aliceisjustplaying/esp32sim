...
pub struct Machine {
    ...
    web_last_frame: u64, web_audio_sent: usize, web_ring_updates: u64, web_last_push_cycles: u64,
    pub console_usb: Vec<u8>, pub console_uart0: Vec<u8>,
    knob_next: u64,
    web_px_pending: u64, web_px_sent: u64,
    rt_last_check: u64,
    pub rt_behind: f64,
    irq_poll: u64,
    pub rt_resyncs: u64,
    pub rt_log: bool, rt_log_last: Option<std::time::Instant>, rt_log_insns: (u64, u64),
    web_cam_pushed: u64, web_cam_sent: bool,
}
...
impl Machine {
    ...
    pub fn boot_app(&mut self, app_off: usize) -> Result<u32, String> {
        ...
        for s in &img.segments {
            ...
            if end > self.bus.flash.len() { return Err("segment beyond flash".into()); }
    ...
    fn drain_console(&mut self) {
        ...
        let mut emit = |bit: u32, tag: &str, d: Vec<u8>, console: &mut Vec<u8>, mask: u32, prefix: bool| {
        ...
        self.console_usb.extend_from_slice(&d); if self.console_usb.len() > 65536 { let cut = self.console_usb.len() - 49152; self.console_usb.drain(..cut); }
        if let Some(w) = &self.web { if !d.is_empty() { w.send_text(&format!("{{\"t\":\"serial\",\"src\":\"usb\",\"data\":\"{}\"}}", crate::web::json_escape(&String::from_utf8_lossy(&d)))); } }
        emit(1, "[usb]  ", d, &mut self.console, self.console_mask, self.console_prefix);
        for u in 0..3 {
            let d = std::mem::take(&mut self.bus.periph.uart[u].tx_out);
            if u == 0 { self.console_uart0.extend_from_slice(&d); if self.console_uart0.len() > 65536 { let cut = self.console_uart0.len() - 49152; self.console_uart0.drain(..cut); } }
            if let Some(w) = &self.web { if !d.is_empty() { w.send_text(&format!("{{\"t\":\"serial\",\"src\":\"uart{}\",\"data\":\"{}\"}}", u, crate::web::json_escape(&String::from_utf8_lossy(&d)))); } }
            let (bit, tag) = if u == 0 { (2, "[uart0] ") } else if u == 1 { (4, "[uart1] ") } else { (4, "[uart2] ") };
            emit(bit, tag, d, &mut self.console, self.console_mask, self.console_prefix);
        }
    }
    ...
    fn step_core(&mut self, core: usize) -> Option<Stop> {
        ...
        if core == self.regtrace_core && !cpu.waiting && self.regtrace.is_some() {
            if !self.regtrace_armed && self.regtrace_from_pc == Some(pc) { self.regtrace_armed = true; }
            if self.regtrace_armed && self.regtrace_count >= self.regtrace_max { return Some(Stop::Halted); }
            if self.regtrace_armed { self.regtrace_count += 1; }
            if let Some(w) = &mut self.regtrace { if !self.regtrace_armed { /* not yet */ } else {
                use std::io::Write;
                let _ = write!(w, "{:08x}", pc);
                for i in 0..16u8 { let _ = write!(w, " {:08x}", cpu.get_ar(i)); }
        ...
        match step(cpu, bus) {
            Ok(()) => {}
            Err(Trap::Exception(c)) => { self.exceptions += 1; if self.trace { eprintln!("          ** core{} exception cause {} at {:08x} -> {:08x} (excvaddr {:08x})", core, c, pc, cpu.pc, cpu.excvaddr); } }
        ...
        if let Some((wa, wv)) = self.watch {
            if let Ok(v) = bus.read32(wa) { if v != wv { self.watch = Some((wa, v)); return Some(Stop::Watch(wa, wv, v)); } }
        }
        if self.exceptions >= self.stop_after_exceptions { return Some(Stop::Exceptions(self.exceptions)); }
        None
    }
    ...
    pub fn run(&mut self, max_insns: u64) -> Stop {
        ...
        loop {
            ...
            if reset { self.core1_was_reset = true; }
            let core1_on = clk && !stall && !reset;
            if core1_on && self.core1_was_reset { self.core1_was_reset = false; self.cpu1.reset(); self.cpu1.prid = 0xABAB; if self.trace { eprintln!("          ** core1 released from reset"); } }
            let c0_idle = self.cpu.waiting && self.cpu.check_interrupts_pending() == 0;
            let c1_idle = !core1_on || (self.cpu1.waiting && self.cpu1.check_interrupts_pending() == 0);
            let slow_path = self.trace || self.profile.is_some() || self.regtrace.is_some() || self.watch.is_some();
            if c0_idle && c1_idle && !slow_path {
                ...
                if core1_on { self.cpu1.advance_ccount(chunk as u32); }
                n += chunk;
    ...
    fn after_round_rest(&mut self) {
        while self.script_pos < self.script.len() && self.script[self.script_pos].0 <= self.bus.cycles {
            let (t, a) = self.script[self.script_pos].clone(); self.script_pos += 1;
            if self.script_log { eprintln!("[script] t={:.3}s {:?}", t as f64 / crate::periph::CPU_HZ as f64, a); }
            match a {
        ...
        if self.realtime && self.bus.cycles.wrapping_sub(self.rt_last_check) >= 1 << 16 {
            ...
            let emulated = std::time::Duration::from_secs_f64(self.bus.cycles as f64 / crate::periph::CPU_HZ as f64);
            let wall = start.elapsed();
            if emulated > wall + std::time::Duration::from_millis(2) { std::thread::sleep(emulated - wall); self.rt_behind = 0.0; }
                ...
                if wall > emulated + std::time::Duration::from_millis(500) { self.rt_resyncs += 1; self.wall_start = Some(std::time::Instant::now() - emulated); }
            } else { self.rt_behind = 0.0; }
        }
    }
    ...
    fn web_push(&mut self) {
        if self.rt_log {
            ...
            self.rt_log_last = Some(now); self.rt_log_insns = (i0, i1);
        }
        ...
        if let Some(tft) = self.bus.board.tft() {
            ...
            if px != self.web_px_pending { self.web_px_pending = px; }
            else if px != self.web_px_sent {
                self.web_px_sent = px; self.web_last_frame = tft.frames;
                let f = tft.frame_160x80();
                let mut b = vec![1u8, 160, 0, 80, 0];
                for p in &f { b.push(*p as u8); b.push((*p >> 8) as u8); }
                w.send_binary(&b);
            }
        }
        if self.web_cam_pushed != self.bus.periph.lcd_cam.frames / 20 || !self.web_cam_sent {
            if let Some(rgb) = self.bus.board.camera_preview(320, 240) {
                let mut b = vec![4u8, (320u16 & 255) as u8, (320u16 >> 8) as u8, (240u16 & 255) as u8, (240u16 >> 8) as u8]; b.extend_from_slice(&rgb); w.send_binary(&b); self.web_cam_sent = true;
            }
        ...
        if let Some(ring) = self.bus.board.ring() { if ring.updates != self.web_ring_updates {
            self.web_ring_updates = ring.updates;
            let leds: Vec<String> = ring.leds.iter().map(|c| format!("[{},{},{}]", c[0], c[1], c[2])).collect();
        ...
        for m in w.poll_incoming() {
            ...
            match t.as_str() {
                "btn" => { let pin: u8 = json_str(&m, "pin").and_then(|x| x.parse().ok()).unwrap_or(0); let v = json_str(&m, "v").unwrap_or_default() == "1";
                           self.bus.periph.gpio.set_input(pin, !v); self.bus.irq_dirty = true; }
                "knobpress" => { let v = json_str(&m, "v").unwrap_or_default() == "1"; self.bus.periph.gpio.set_input(PIN_ENC_SW, !v); self.bus.irq_dirty = true; }
                "knob" => {
                    let d: i32 = json_str(&m, "d").and_then(|x| x.parse().ok()).unwrap_or(1);
                    let hz = crate::periph::CPU_HZ; let step = hz / 500;   // 2 ms per phase
                    let mut tc = (self.bus.cycles + step).max(self.knob_next);   // queue detents back to back, never overlapping
                    let seq: [(u8, bool); 4] = if d > 0 { [(PIN_ENC_CLK, false), (PIN_ENC_DT, false), (PIN_ENC_CLK, true), (PIN_ENC_DT, true)] } else { [(PIN_ENC_DT, false), (PIN_ENC_CLK, false), (PIN_ENC_DT, true), (PIN_ENC_CLK, true)] };
                    for _ in 0..d.unsigned_abs() { for (pn, l) in seq { self.script.push((tc, ScriptAction::Gpio(pn, l))); tc += step; } tc += step * 4; }
                    self.knob_next = tc;
    ...
    pub fn write_wav(&self, path: &str) -> std::io::Result<usize> {
        let a = self.bus.periph.audio(); let pcm = &a.pcm; let rate = a.sample_rate;
    ...
    pub fn irq_report(&self) -> String {
        ...
        for core in 0..2 {
            for irq in 0..32 {
                ...
                if n == 0 { continue; }
                let srcs: Vec<String> = (0..crate::periph::NUM_SOURCES).filter(|&src| self.bus.periph.intmatrix.map[core][src] == irq as u32).map(|src| src.to_string()).collect();
    ...
    pub fn write_gram_png(&self, path: &str) -> std::io::Result<()> {
        ...
        write_png_rgb565(path, &t.gram, crate::board::St7735::COLS, crate::board::St7735::ROWS, 2)
    }
    ...
    pub fn load_script(&mut self, text: &str) -> Result<(), String> {
        ...
        for (ln, line) in text.lines().enumerate() {
            ...
            let pin = |s: &str| -> Result<u8, String> { match s { "btn1" => Ok(PIN_BTN1), "btn2" => Ok(PIN_BTN2), "sw" | "knob" => Ok(PIN_ENC_SW), _ => s.parse().map_err(|_| format!("line {}: bad pin", ln + 1)) } };
            let c = (t * hz) as u64;
            match cmd {
                ...
                "knob" => {
                    ...
                    for _ in 0..n {
                        ...
                        let seq: [(u8, bool); 4] = if dir == "cw" { [(PIN_ENC_CLK, false), (PIN_ENC_DT, false), (PIN_ENC_CLK, true), (PIN_ENC_DT, true)] } else { [(PIN_ENC_DT, false), (PIN_ENC_CLK, false), (PIN_ENC_DT, true), (PIN_ENC_CLK, true)] };
                        for (pn, l) in seq { ev.push((tc, ScriptAction::Gpio(pn, l))); tc += step; }
                        tc += step * 4;
                    }
                }
        ...
        self.script = ev; self.script_pos = 0;
        Ok(())
    }

    pub fn profile_report(&self, top: usize) -> String {
        let Some(h) = &self.profile else { return String::new() };
    ...
    pub fn dump_regs(&self) -> String {
        ...
        if !self.core1_was_reset { out += &self.dump_core(&self.cpu1, 1); }
        out
    }
...
pub fn write_png_rgb565(path: &str, px: &[u16], w: usize, h: usize, scale: usize) -> std::io::Result<()> {
    fn crc32(data: &[u8]) -> u32 { let mut c = 0xffff_ffffu32; for &b in data { c ^= b as u32; for _ in 0..8 { c = if c & 1 != 0 { 0xEDB8_8320 ^ (c >> 1) } else { c >> 1 }; } } !c }
    fn adler(data: &[u8]) -> u32 { let (mut a, mut b) = (1u32, 0u32); for &d in data { a = (a + d as u32) % 65521; b = (b + a) % 65521; } (b << 16) | a }
    fn chunk(out: &mut Vec<u8>, t: &[u8], d: &[u8]) { out.extend_from_slice(&(d.len() as u32).to_be_bytes()); let mut td = t.to_vec(); td.extend_from_slice(d); out.extend_from_slice(&td); out.extend_from_slice(&crc32(&td).to_be_bytes()); }
    let (W, H) = (w * scale, h * scale);
    let mut raw = Vec::with_capacity(H * (W * 3 + 1));
    for y in 0..H { raw.push(0); for x in 0..W { let p = px[(y / scale) * w + x / scale] as u32; raw.push(((p >> 11) * 255 / 31) as u8); raw.push((((p >> 5) & 63) * 255 / 63) as u8); raw.push(((p & 31) * 255 / 31) as u8); } }
    // zlib stream with stored (uncompressed) deflate blocks
    let mut z = vec![0x78, 0x01];
    for (i, blk) in raw.chunks(65535).enumerate() { let last = (i + 1) * 65535 >= raw.len(); z.push(last as u8); z.extend_from_slice(&(blk.len() as u16).to_le_bytes()); z.extend_from_slice(&(!(blk.len() as u16)).to_le_bytes()); z.extend_from_slice(blk); }
    z.extend_from_slice(&adler(&raw).to_be_bytes());
    let mut out = vec![137, 80, 78, 71, 13, 10, 26, 10];
    let mut ihdr = Vec::new(); ihdr.extend_from_slice(&(W as u32).to_be_bytes()); ihdr.extend_from_slice(&(H as u32).to_be_bytes()); ihdr.extend_from_slice(&[8, 2, 0, 0, 0]);
    chunk(&mut out, b"IHDR", &ihdr); chunk(&mut out, b"IDAT", &z); chunk(&mut out, b"IEND", &[]);
    std::fs::write(path, out)
}
