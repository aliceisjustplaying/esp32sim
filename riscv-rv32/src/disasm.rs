...
pub fn format(i: &Insn) -> String {
    ...
    if matches!(
        i.comp,
        Comp::Li | Comp::Slli | Comp::Mv | Comp::Add | Comp::Lui
    ) && i.rd == 0
    {
        return format_compressed(i);
    }
    if i.comp == Comp::Nop && i.imm != 0 {
        return format_compressed(i);
    }
    ...
    match i.op {
        ...
        Jal => {
            if i.rd == 0 {
                format!("j\t{:x}", i.imm as u32)
            } else if i.rd == 1 {
                format!("jal\t{:x}", i.imm as u32)
            } else {
                format!("jal\t{},{:x}", rd, i.imm as u32)
            }
        }
        ...
        Beq | Bne | Blt | Bge | Bltu | Bgeu => {
            ...
            if i.rs2 == 0 {
                ...
                if let Some(z) = z {
                    return format!("{}\t{},{:x}", z, rs1, i.imm as u32);
                }
            }
            if i.rs1 == 0 && matches!(i.op, Blt | Bge) {
                return format!(
                    "{}\t{},{:x}",
                    if i.op == Blt { "bgtz" } else { "blez" },
                    rs2,
                    i.imm as u32
                );
            }
        ...
        Xori => {
            if i.imm == -1 {
                ...
                format!("xori\t{},{},{}", rd, rs1, i.imm)
            }
        }
        Ori => format!("ori\t{},{},{}", rd, rs1, i.imm),
        Andi => {
            if i.imm == 255 {
                format!("zext.b\t{},{}", rd, rs1)
            } else {
                format!("andi\t{},{},{}", rd, rs1, i.imm)
            }
        }
