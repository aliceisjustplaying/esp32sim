...
pub mod csr {
    ...
    pub const MPCER: u32 = 0x7E0;
    pub const MPCMR: u32 = 0x7E1;
    pub const MPCCR: u32 = 0x7E2;
    pub const PCER_U: u32 = 0x800;
    pub const PCMR_U: u32 = 0x801;
    pub const PCCR_U: u32 = 0x802;
}

pub mod exc {
    pub const INSN_MISALIGNED: u32 = 0;
    pub const INSN_ACCESS_FAULT: u32 = 1;
...
impl Cpu {
    ...
    pub fn write_csr(&mut self, n: u32, v: u32) {
        ...
        match n {
            ...
            MTVAL => self.mtval = v,
            MCYCLE | MINSTRET | MPCCR | PCCR_U => {
                self.insn_count = (self.insn_count & !0xffff_ffff) | v as u64
            }
            MCYCLEH | MINSTRETH => {
                self.insn_count = (self.insn_count & 0xffff_ffff) | ((v as u64) << 32)
            }
            MISA | MIP | MVENDORID | MARCHID | MIMPID | MHARTID => {} // read-only
            _ => {
                self.csr_other.insert(n, v);
            }
        }
    }
    ...
    pub fn trap(&mut self, cause: u32, tval: u32, epc: u32) {
        ...
        self.mcause = cause;
        self.mtval = tval;
        let mpie = if self.mstatus & mstatus::MIE != 0 {
            mstatus::MPIE
        } else {
            0
        };
    ...
    pub fn mret(&mut self) {
        ...
        self.mstatus =
            (self.mstatus & !mstatus::MIE) | if mpie { mstatus::MIE } else { 0 } | mstatus::MPIE;
        self.pc = self.mepc;
    }
}
