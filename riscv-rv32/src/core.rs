...
impl emu_core::Core for Cpu {
    ...
    fn set_irq(&mut self, line: Option<u32>) {
        self.irq = line;
    }
    fn irq_pending(&self) -> bool {
        self.irq.is_some()
    }
    fn irq_bits(irq: &Option<u32>) -> u32 {
        irq.map_or(0, |l| 1 << (l & 31))
    }
    fn idle_advance(&mut self, cycles: u32) {
        self.insn_count += cycles as u64;
    }
    ...
    fn run<B: Bus>(&mut self, bus: &mut B, budget: u32) -> (u32, Option<Trap>) {
        for i in 0..budget {
            if let Err(t) = crate::exec::step(self, bus) {
                return (i + 1, Some(t));
            }
            if bus.block_break() {
                return (i + 1, None);
    ...
    fn disasm(&self, pc: u32, bytes: [u8; 4]) -> String {
        crate::disasm::format(&crate::decode::decode(pc, bytes)).replace('\t', " ")
    }
    ...
    fn trace_regs(&self) -> String {
        format!(
            "ra={:08x} sp={:08x} a0={:08x} a1={:08x}",
            self.x[1], self.x[2], self.x[10], self.x[11]
        )
    }
    fn regtrace_line(&self, pc: u32) -> String {
        let mut s = format!("{:08x}", pc);
        for i in 1..32 {
            s += &format!(" {:08x}", self.x[i]);
        }
...
mod tests {
    ...
    fn core_runs_steps() {
        ...
        assert_eq!((used, trap), (4, None));
        assert_eq!(cpu.x[1], 5);
        assert_eq!(Core::pc(&cpu), 0x4038_0004);
        let mut r = Vec::new();
        cpu.regs(&mut r);
        assert_eq!(r[0], ("ra", 5));
    }
