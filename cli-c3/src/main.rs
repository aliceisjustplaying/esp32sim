...
fn main() {
    ...
    let (mut boot, mut rom, mut bootloader, mut ptable, mut app) = ("rom".to_string(), None::<String>, None, None, None);
    let (mut elfs, mut flash_mb, mut flash_image): (Vec<String>, usize, Option<String>) = (Vec::new(), 4, None);
    let (mut max_seconds, mut max_insns, mut console) = (None::<f64>, u64::MAX, "uart0".to_string());
    ...
    let pair = |s: &str| -> (u32, usize) {
        match s.split_once(',') { Some((a, n)) => (num(a), n.parse().unwrap_or(1)), None => (num(s), 1) }
    };
    while i < args.len() {
        let a = args[i].clone();
        let mut next = || { i += 1; args.get(i).cloned().unwrap_or_else(|| usage()) };
        match a.as_str() {
            "--boot" => boot = next(),
            "--rom" => rom = Some(next()),
            ...
            "--break" => { let v = next(); breaks.push(num(&v)); }
            "--log-periph" => log_periph = true,
            "--peek" => { let v = next(); peeks.push(pair(&v)); }
    ...
    if boot == "rom" {
        match &rom_path {
            Some(p) => match std::fs::read(p) {
                Ok(d) => { m.load_rom(&d).expect("rom"); eprintln!("[emu] ROM loaded from {}", p.display()); }
                Err(e) => { eprintln!("[emu] {}: {}", p.display(), e); std::process::exit(2) }
            },
            None => { eprintln!("[emu] no esp32c3 mask ROM ELF found (pass --rom, or use --boot app)"); std::process::exit(2) }
        }
    }
    ...
    for spec in &flash_at {
        let Some((off, path)) = spec.split_once('=') else { eprintln!("--flash-at needs OFFSET=FILE"); std::process::exit(2) };
        let off = usize::from_str_radix(off.trim_start_matches("0x"), 16).unwrap_or_else(|_| { eprintln!("--flash-at: bad offset"); std::process::exit(2) });
    ...
    if let Some(s) = &serial { m.bus.periph.usb.host_input(s.as_bytes()); }
