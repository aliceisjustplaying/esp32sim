...
impl<S: Soc> Machine<S> {
    ...
    fn vq_quanta(&self, insns_left: u64, on: &[bool], busy: usize) -> u64 {
        ...
        if self.web.is_some() { k = k.min(((S::CPU_HZ / self.bus.board_ref().display_push_hz().max(1)).max(1)).saturating_sub(now.wrapping_sub(self.ws.last_push_cycles)).div_ceil(QUANTUM)); }
        k.max(1)
    }
