...
impl<S: Soc> Observer<S> for Coverage {
    ...
    fn on_block(&mut self, _cx: &Ctx, _core: usize, pc: u32, _insns: u32) {
        self.starts.insert(pc);
    }
    fn on_insn(
        &mut self,
        _cx: &Ctx,
    ...
    fn report(&mut self, cx: &Ctx) -> String {
        ...
        for &pc in &self.starts {
            let s = cx.sym(pc);
            *per_sym
                .entry(s.split('+').next().unwrap_or("").to_string())
                .or_insert(0) += 1;
        }
        let named = per_sym.iter().filter(|(k, _)| !k.is_empty()).count();
        let mut s = format!(
            "[coverage] {} block starts in {} functions\n",
            self.starts.len(),
            named
        );
        if let Some(p) = &self.path {
            ...
            match std::fs::write(p, text) {
                Ok(()) => s += &format!("[coverage] wrote {}\n", p),
                Err(e) => s += &format!("[coverage] {}: {}\n", p, e),
            }
        }
        s
    }
