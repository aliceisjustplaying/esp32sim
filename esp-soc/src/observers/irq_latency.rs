...
pub struct IrqLatency {
    raised: Vec<[Option<u64>; 32]>,
    stat: Vec<[Stat; 32]>,
}
impl IrqLatency {
    pub fn new(cores: usize) -> Self {
        IrqLatency {
            raised: vec![[None; 32]; cores],
            stat: vec![[Stat::default(); 32]; cores],
        }
    }
}
impl<S: Soc> Observer<S> for IrqLatency {
    ...
    fn report(&mut self, cx: &Ctx) -> String {
        let mut s = String::from(
            "[irq-latency] per core, line: taken, mean cycles, max cycles (raised -> taken)\n",
        );
        for (core, st) in self.stat.iter().enumerate() {
            for (line, x) in st.iter().enumerate() {
                ...
                s += &format!(
                    "  core{} int{:<2} {:>9}  mean {:>8.1}  max {:>8}  ({:.2} us max)\n",
                    core,
                    line,
                    x.n,
                    x.total as f64 / x.n as f64,
                    x.max,
                    x.max as f64 * 1e6 / cx.cpu_hz as f64
                );
            }
        }
        s
