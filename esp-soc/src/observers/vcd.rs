...
impl Vcd {
    pub fn new(path: &str, cpu_hz: u64) -> Self {
        Vcd {
            path: path.to_string(),
            cpu_hz,
            events: Vec::new(),
    ...
    fn ps(&self, cycles: u64) -> u64 {
        (cycles as u128 * 1_000_000_000_000u128 / self.cpu_hz as u128) as u64
    }
    /// signal ids: 0..=127 GPIO pins, 128 + core*32 + line: interrupt line raised/taken
    fn push(&mut self, cycle: u64, id: u16, level: bool) {
        if (id as usize) < 256 {
            self.seen[id as usize] = true;
        }
        self.events.push((cycle, id, level));
    }
}
impl<S: Soc> Observer<S> for Vcd {
    ...
    fn report(&mut self, _cx: &Ctx) -> String {
        let Ok(f) = std::fs::File::create(&self.path) else {
            return format!("[vcd] cannot write {}", self.path);
        };
        ...
        for id in 0..256u16 {
            ...
            let name = if id < 128 {
                ...
                format!("core{}_int{}", (id - 128) / 32, (id - 128) % 32)
            };
        ...
        for id in 0..256u16 {
            if self.seen[id as usize] {
                let _ = writeln!(f, "{}{}", if id < 128 { 'x' } else { '0' }, code(id));
            }
        }
