...
impl Soc for C3 {
    ...
    fn new_core(_i: usize) -> Cpu {
        Cpu::new()
    }
    fn reset_core(c: &mut Cpu, _i: usize) {
        Cpu::reset(c);
    }
    ...
    fn irqs(bus: &SocBus, out: &mut [Option<u32>]) {
        out[0] = bus.periph.intc.pending();
    }
}
...
impl esp_soc::SocBus for SocBus {
    ...
    fn irq_sources_of(&self, _core: usize, line: u32) -> Vec<usize> {
        (0..src::COUNT)
            .filter(|&s| self.periph.intc.map[s] == line)
            .collect()
    }
}
