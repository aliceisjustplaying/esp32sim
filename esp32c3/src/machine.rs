...
impl Machine {
    ...
    pub fn load_rom(&mut self, rom_elf: &[u8]) -> Result<(), String> {
        ...
        if let (Some(ds), Some(de)) = (find("_data_end_btdm_rom").or_else(|| find("_data_start")), find("_data_end")) {
            let mut t = ds;
            let mut n = 0;
            while t + 16 <= de {
    ...
    pub fn boot_rom(&mut self) { self.cpu.reset(); }
    ...
    pub fn sym(&self, pc: u32) -> String {
        match self.symbols.range(..=pc).next_back() {
            Some((&a, n)) if pc - a < 0x10000 => if a == pc { n.clone() } else { format!("{}+{:#x}", n, pc - a) },
            _ => String::new(),
        }
    }

    pub fn sym_addr(&self, name: &str) -> Option<u32> {
        self.symbols.iter().find(|(_, n)| n.as_str() == name).map(|(a, _)| *a)
    }
    ...
    pub fn run(&mut self, max_insns: u64) -> Stop {
        ...
        loop {
            ...
            if self.cpu.waiting && !slow && self.cpu.irq.is_none() {
                ...
                for _ in 0..QUANTUM {
                    if let Some(stop) = self.step_one() { self.drain_console(); return stop; }
                }
            }
            n += QUANTUM;
    ...
    fn step_one(&mut self) -> Option<Stop> {
        ...
        if self.trace && self.cpu.insn_count >= self.trace_from {
            if let Ok(b) = self.bus.fetch(pc) {
                ...
                eprintln!("{:>10} {:08x}: {:<28} {}  ra={:08x} sp={:08x} a0={:08x} a1={:08x}",
                          self.cpu.insn_count, pc, riscv_rv32::disasm::format(&i).replace('\t', " "), self.sym(pc),
                          self.cpu.x[1], self.cpu.x[2], self.cpu.x[10], self.cpu.x[11]);
            }
        }
        match step(&mut self.cpu, &mut self.bus) {
            Ok(()) => {}
            Err(Trap::Interrupt(line)) => { self.interrupts += 1; self.irq_hist[(line & 31) as usize] += 1; }
        ...
        if let Some((wa, wv)) = self.watch {
            if let Ok(v) = self.bus.read32(wa) {
                if v != wv { self.watch = Some((wa, v)); return Some(Stop::Watch(wa, wv, v)); }
            }
        }
    ...
    pub fn drain_console(&mut self) {
        ...
        if !usb.is_empty() {
            ...
            if self.console_mask & 1 != 0 { self.console.extend_from_slice(&usb); }
        }
    ...
    pub fn seconds(&self) -> f64 { self.bus.cycles as f64 / CPU_HZ as f64 }
}
