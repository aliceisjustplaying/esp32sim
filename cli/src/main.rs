...
fn find_rom() -> Option<PathBuf> {
    ...
    for e in dir.flatten() {
        let p = e.path().join("esp32s3_rev0_rom.elf");
        if p.exists() {
            return Some(p);
        }
    }
...
fn main() {
    ...
    let mut rom = find_rom();
    let mut bootloader = None;
    let mut ptable = None;
    let mut app = None;
    let mut elfs: Vec<String> = Vec::new();
    let mut boot = "app".to_string();
    let mut max_insns = u64::MAX;
    let mut trace = false;
    let mut trace_from = 0u64;
    let mut breaks = Vec::new();
    let mut log_periph = false;
    let mut dump_at_end = true;
    let mut peeks: Vec<(u32, usize)> = Vec::new();
    let mut disasms: Vec<(u32, usize)> = Vec::new();
    let mut watch = None;
    let mut stop_exc = u64::MAX;
    let mut profile = false;
    let mut wav: Option<String> = None;
    let mut tft_png: Option<String> = None;
    let mut gram_png: Option<String> = None;
    let mut script: Option<String> = None;
    let mut max_seconds: Option<f64> = None;
    let mut console = "both".to_string();
    let mut console_prefix = false;
    let mut regtrace: Option<String> = None;
    let mut regtrace_max = u64::MAX;
    let mut regtrace_from_pc: Option<u32> = None;
    let mut efuse_file: Option<String> = None;
    let mut regs_init: Option<String> = None;
    let mut web_port: Option<u16> = None;
    let mut realtime = false;
    let mut web_dir: Option<String> = None;
    let mut strap: Option<u32> = None;
    let mut reset_cause: Option<u32> = None;
    let mut flash_mb: usize = 8;
    let mut flash_image: Option<String> = None;
    let mut board = "atech14".to_string();
    let mut no_reboot = false;
    let mut psram_mb: usize = 2;
    let mut cam_image: Option<String> = None;
    let mut cam_fps: f64 = 10.0;
    let mut stubs: Vec<String> = Vec::new();
    let mut regstat: Option<String> = None;
    let mut trace_fns: Vec<String> = Vec::new();
    ...
    while i < args.len() {
        ...
        match a {
            ...
            "--trace-from" => {
                trace = true;
                trace_from = next().replace('_', "").parse().expect("trace-from")
            }
            "--break" => breaks.push(
                u32::from_str_radix(next().trim_start_matches("0x"), 16).expect("break addr"),
            ),
            ...
            "--watch" => {
                watch = Some(
                    u32::from_str_radix(next().trim_start_matches("0x"), 16).expect("watch addr"),
                )
            }
            ...
            "--regtrace-from-pc" => {
                regtrace_from_pc =
                    Some(u32::from_str_radix(next().trim_start_matches("0x"), 16).expect("pc"))
            }
            ...
            "--strap" => {
                strap =
                    Some(u32::from_str_radix(next().trim_start_matches("0x"), 16).expect("strap"))
            }
            "--reset-cause" => {
                reset_cause =
                    Some(u32::from_str_radix(next().trim_start_matches("0x"), 16).expect("cause"))
            }
    ...
    if regstat.is_some() {
        m.bus.periph.regstat = Some(Default::default());
    }
    if let Some(spec) = &wifi {
        ...
        eprintln!(
            "[emu] virtual AP '{}' bssid {} channel {} ({})",
            cfg.ssid,
            esp32s3::wifi::mac_str(&cfg.bssid),
            cfg.channel,
            if cfg.psk.is_some() {
                "WPA2-PSK"
            } else {
        ...
        if net_mode == "nat" || net_mode == "user" {
            ...
            eprintln!(
                "[emu] NAT to the host network enabled (DNS via {}.{}.{}.{})",
                nat.resolver[0], nat.resolver[1], nat.resolver[2], nat.resolver[3]
            );
            net.nat = Some(nat);
        }
    ...
    if let Some(p) = &cam_image {
        match esp32s3::picture::load(p) {
            Ok(pic) => {
                eprintln!("[emu] camera picture {} ({}x{})", p, pic.w, pic.h);
                m.bus.board.set_camera_picture(pic);
            }
            Err(e) => {
                eprintln!("[emu] {}", e);
                std::process::exit(2);
    ...
    for spec in &flash_at {
        ...
        let off = usize::from_str_radix(off.trim_start_matches("0x"), 16).unwrap_or_else(|_| {
            eprintln!("--flash-at: bad offset {}", off);
            std::process::exit(2)
        });
        ...
        m.write_flash(off, &data).unwrap_or_else(|e| {
            eprintln!("--flash-at: {}", e);
            std::process::exit(2)
        });
    ...
    if no_jit {
        m.cpu.blocks.jit_enabled = false;
        m.cpu1.blocks.jit_enabled = false;
    }
    m.trace = trace;
    ...
    for pre in &trace_fns {
        let mut n = 0;
        for (&a, name) in m.symbols.iter() {
            if name.starts_with(pre.as_str())
                || (pre.ends_with('$') && name == &pre[..pre.len() - 1])
            {
                m.fn_probes.insert(a, name.clone());
                n += 1;
            }
        }
    ...
    for st in &stubs {
        let (name, val) = match st.split_once('=') {
            Some((n, v)) => (
                n,
                u32::from_str_radix(
        ...
        let addr = if let Some(a) = m.sym_addr(name) {
            a
        } else if let Ok(a) = u32::from_str_radix(name.trim_start_matches("0x"), 16) {
            a
        } else {
            eprintln!("--stub: unknown symbol {}", name);
            std::process::exit(2)
        };
    ...
    match boot.as_str() {
        "app" => {
            let entry = m.boot_app(0x10000).expect("boot app");
            eprintln!("[emu] app boot: entry {:#010x} {}", entry, m.sym(entry));
        }
        "rom" => {
            m.boot_rom();
    ...
    if let Some(wa) = watch {
        let v = xtensa_lx7::bus::Bus::read32(&mut m.bus, wa).unwrap_or(0);
        m.watch = Some((wa, v));
    }
    ...
    if let Some(p) = &regtrace {
        m.regtrace = Some(std::io::BufWriter::new(
            std::fs::File::create(p).expect("regtrace file"),
        ));
        m.regtrace_max = regtrace_max;
        if let Some(a) = regtrace_from_pc {
            m.regtrace_from_pc = Some(a);
            m.regtrace_armed = false;
        }
    }
    if let Some(p) = &efuse_file {
        ...
        for line in txt.lines() {
            ...
            let Ok(mut a) = u32::from_str_radix(addr_s.trim().trim_start_matches("0x"), 16) else {
                continue;
            };
            for w in rest.split_whitespace() {
                if let Ok(v) = u32::from_str_radix(w, 16) {
                    let off = if a >= 0x6000_7000 { a - 0x6000_7000 } else { a };
                    m.bus.periph.efuse.ram.write(off, v);
                    a += 4;
                    n += 1;
                }
            }
    ...
    if let Some(p) = &regs_init {
        ...
        let mut n = 0;
        for line in txt.lines() {
            let (addr_s, rest) = match line.trim().split_once(':') {
                Some(x) => x,
                None => continue,
            };
            let Ok(mut a) = u32::from_str_radix(addr_s.trim().trim_start_matches("0x"), 16) else {
    ...
    if let Some(port) = web_port {
        ...
        eprintln!(
            "[emu] board UI: http://127.0.0.1:{}/  (serving {})",
            port, dir
        );
        m.web = Some(w);
        m.realtime = true;
    ...
    if let Some(p) = &script {
        m.load_script(&std::fs::read_to_string(p).expect("script"))
            .expect("script");
    }
    if let Some(sec) = max_seconds {
    ...
    if let Stop::Unimplemented(pc, raw) = stop {
        if let Ok(b) = xtensa_lx7::bus::Bus::fetch(&mut m.bus, pc) {
            let ins = xtensa_lx7::decode(pc, b);
            eprintln!(
                "[emu] unimplemented at {:08x} {}: {} (raw {:#x})",
                pc,
                m.sym(pc),
                xtensa_lx7::disasm::format(&ins),
                raw
            );
        }
    }
    ...
    if profile {
        eprintln!("{}", m.profile_report(12));
    }
    eprintln!("{}", m.irq_report());
    if let Some(w) = &wav {
        match m.write_wav(w) {
            Ok(n) => eprintln!(
                "[emu] wrote {} samples ({:.2} s) to {}",
                n,
                n as f64 / m.bus.periph.audio().sample_rate as f64,
                w
            ),
            Err(e) => eprintln!("[emu] wav: {}", e),
        }
    }
    eprintln!(
        "[emu] i2s frames out: {} (i2s0 @ {} Hz) {} (i2s1 @ {} Hz)",
        m.bus.periph.i2s0.frames_out,
        m.bus.periph.i2s0.sample_rate,
        m.bus.periph.i2s1.frames_out,
        m.bus.periph.i2s1.sample_rate
    );
    if let Some(t) = m.bus.board.tft() {
        eprintln!("[emu] tft: {} RAMWR, {} pixels, madctl={:#x} inverted={} on={} bbox={:?} top colours {:x?}; gpio events {}", t.frames, t.pixels_written, t.madctl, t.inverted, t.on, t.bbox(), t.histogram(5), m.bus.board.gpio_events());
    }
    if let (Some(path), Some(st)) = (&regstat, &m.bus.periph.regstat) {
        ...
        for (&(addr, pc, wr), &(n, val)) in rows {
            ...
            let _ = writeln!(
                f,
                "{} {} {}+0x{:03x} {:#010x} {:#010x} {:#010x} {}",
                n,
                if wr { "wr" } else { "rd" },
                esp32s3::periph::Peripherals::block_name_pub(block),
                addr & 0xfff,
                addr,
                val,
                pc,
    ...
    }
    {
        ...
        if w.tx_frames + w.rx_frames > 0 {
            eprintln!("[emu] wifi: {} frames sent by the station, {} received ({} dropped: no descriptor){}", w.tx_frames, w.rx_frames, w.rx_dropped, w.ap.as_ref().map_or(String::new(), |ap| format!("; AP: {} beacons, {} probe responses, {} data frames from the station, state {:?}", ap.stats.0, ap.stats.1, ap.stats.2, ap.state)));
        }
        if let Some(n) = &w.net {
            eprintln!("[emu] net: {} DHCP leases, {} ARP replies, {} DNS answers, {} NTP answers, {} TCP refused, {} pings, {} frames ignored", n.dhcp_acks, n.arp_replies, n.dns_answers, n.ntp_answers, n.tcp_rejects, n.pings, n.unhandled);
            if let Some(t) = &n.nat {
                eprintln!("[emu] nat: {} TCP connections ({} failed), {} UDP flows, {} bytes out, {} bytes in", t.tcp_opened, t.tcp_refused, t.udp_flows, t.bytes_to_host, t.bytes_to_guest);
            }
        }
    }
    {
        let (a, sh, r) = (&m.bus.periph.aes, &m.bus.periph.sha, &m.bus.periph.rsa);
        if a.blocks + sh.blocks + r.ops > 0 {
            eprintln!(
                "[emu] crypto: {} AES blocks, {} SHA blocks, {} RSA/MPI operations",
    ...
    }
    {
        ...
        if b0.builds + b1.builds > 0 {
            eprintln!("[emu] blocks: {} built ({} cache flushes) core0, {} ({}) core1; jit: {} compiled, {} KB code", b0.builds, b0.flushes, b1.builds, b1.flushes, b0.compiled + b1.compiled, (b0.code_bytes() + b1.code_bytes()) / 1024);
        }
    }
    if m.stub_hits > 0 {
        eprintln!("[emu] stubs hit {} times", m.stub_hits);
    }
    if m.bus.periph.lcd_cam.lcd_frames > 0 {
        eprintln!("[emu] lcd: {} RGB frames", m.bus.periph.lcd_cam.lcd_frames);
    }
    if m.bus.periph.lcd_cam.frames + m.bus.periph.lcd_cam.dropped > 0 {
        eprintln!(
            "[emu] camera: {} frames delivered, {} dropped (no DMA/no picture)",
            m.bus.periph.lcd_cam.frames, m.bus.periph.lcd_cam.dropped
        );
    }
    if let Some(r) = m.bus.board.ring() {
        eprintln!(
            "[emu] ring: {} updates, leds {:?}; rmt tx {}",
            r.updates,
            &r.leds[..4],
            m.bus.periph.rmt.tx_count
        );
    }
    if let Some(p) = &tft_png {
        match m.write_tft_png(p, 3) {
            Ok(()) => eprintln!("[emu] wrote {}", p),
            Err(e) => eprintln!("[emu] png: {}", e),
        }
    }
    if let Some(p) = &gram_png {
        match m.write_gram_png(p) {
            Ok(()) => eprintln!("[emu] wrote {}", p),
            Err(e) => eprintln!("[emu] png: {}", e),
        }
    }
    if dump_at_end {
        eprintln!("{}", m.dump_regs());
    }
}
