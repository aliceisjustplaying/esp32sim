...
use esp_soc::observers::{
    BlockProfile, Breakpoints, Coverage, IrqLatency, MmioHeat, PcHist, RegTrace, Trace, Vcd, Watch,
};
...
fn usage(chip: &str) -> ! {
    ...
    eprintln!(
        "       see docs/cli.md for every flag (default chip here: {})",
        chip
    );
    std::process::exit(2)
}

fn hex(s: &str, what: &str) -> u32 {
    u32::from_str_radix(s.trim_start_matches("0x"), 16).unwrap_or_else(|_| {
        eprintln!("--{}: bad hex {}", what, s);
        std::process::exit(2)
    })
}
fn pair(s: &str, dflt: usize) -> (u32, usize) {
    match s.split_once(',') {
        Some((a, n)) => (hex(a, "addr"), n.parse().unwrap_or(dflt)),
        None => (hex(s, "addr"), dflt),
    }
}
...
pub struct Opts {
    ...
    pub elfs: Vec<String>,
    pub flash_image: Option<String>,
    pub flash_at: Vec<String>,
    pub boot: Option<String>,
    pub flash_mb: Option<usize>,
    pub psram_mb: Option<usize>,
    pub mac: Option<[u8; 6]>,
    pub strap: Option<u32>,
    pub reset_cause: Option<u32>,
    pub efuse_regs: Option<String>,
    pub regs_init: Option<String>,
    pub board: String,
    pub wifi: Option<String>,
    pub net: String,
    pub cam_image: Option<String>,
    pub cam_fps: f64,
    pub max_insns: u64,
    ...
    pub tft_png: Option<String>,
    pub gram_png: Option<String>,
    pub dump: bool,
    pub trace: bool,
    pub trace_from: u64,
    pub breaks: Vec<u32>,
    pub watch: Option<u32>,
    pub peeks: Vec<(u32, usize)>,
    pub disasms: Vec<(u32, usize)>,
    pub profile: bool,
    pub profile_blocks: bool,
    pub coverage: Option<Option<String>>,
    pub irq_latency: bool,
    pub vcd: Option<String>,
    pub regstat: Option<String>,
    pub regtrace: Option<String>,
...
pub fn parse(args: &[String], default_chip: &str) -> Opts {
    ...
    while i < args.len() {
        ...
        let mut next = || {
            i += 1;
            args.get(i).cloned().unwrap_or_else(|| usage(default_chip))
        };
        match a {
            ...
            "--trace-from" => {
                o.trace = true;
                o.trace_from = next().replace('_', "").parse().expect("trace-from")
            }
...
fn find_rom(name: &str) -> Option<PathBuf> {
    ...
    let mut dirs: Vec<PathBuf> =
        std::fs::read_dir(format!("{}/.espressif/tools/esp-rom-elfs", home))
            .ok()?
            .flatten()
            .map(|e| e.path())
            .collect();
...
fn setup_s3(o: &Opts) -> esp32s3::Machine {
    ...
    m.bus.board = esp32s3::board::make_board(&o.board).unwrap_or_else(|| {
        eprintln!(
    ...
    if !o.debug.is_empty() {
        let mut f = esp_soc::DebugFlags::from_env();
        for d in &o.debug {
            f.parse(d);
        }
        m.set_debug(&f);
    ...
    if let Some(p) = &o.cam_image {
        match esp_soc::picture::load(p) {
            Ok(pic) => {
    ...
    if let Some(mb) = o.flash_mb {
        if mb != 8 {
            m.bus.set_flash_size(mb << 20);
        }
    }
    ...
    if let Some(p) = &o.regstat {
        m.add_observer(Box::new(MmioHeat::new(p, |a| {
            let b = a.wrapping_sub(esp32s3::periph::PERIPH_BASE) >> 12;
            format!(
                "{}+0x{:03x}",
                esp32s3::periph::Peripherals::block_name_pub(b),
                a & 0xfff
            )
        })));
    }
...
fn setup_c3(o: &Opts) -> esp32c3::Machine {
    let mut m = esp32c3::machine(
        o.mac.unwrap_or([0x60, 0x55, 0xf9, 0x00, 0x11, 0x22]),
        o.flash_mb.unwrap_or(4) << 20,
    );
    ...
    for (flag, on) in [
        ("--board", o.board != "atech14" && o.board != "none"),
        ("--wifi", o.wifi.is_some()),
        ("--cam-image", o.cam_image.is_some()),
        ("--psram-mb", o.psram_mb.is_some()),
        ("--efuse-regs", o.efuse_regs.is_some()),
        ("--regs-init", o.regs_init.is_some()),
        ("--regstat", o.regstat.is_some()),
    ] {
        if on {
            eprintln!("{} is not available on the C3", flag);
            std::process::exit(2);
        }
    }
    m
...
fn run<S: Soc>(mut m: Machine<S>, o: &Opts) {
    ...
    let boot = o
        .boot
        .clone()
        .unwrap_or_else(|| if c3 { "rom".into() } else { "app".into() });
    let console = o
        .console
        .clone()
        .unwrap_or_else(|| if c3 { "uart0".into() } else { "both".into() });
    m.bus.misc().log_unknown = o.log_periph;
    if !o.breaks.is_empty() {
        m.add_observer(Box::new(Breakpoints {
            pcs: o.breaks.clone(),
        }));
    }
    if o.trace {
        m.add_observer(Box::new(Trace { from: o.trace_from }));
    }
    ...
    match &rom {
        ...
        None if boot == "rom" => {
            eprintln!(
                "[emu] no {} mask ROM ELF found (pass --rom, or use --boot app)",
                S::NAME
            );
            std::process::exit(2)
        }
    ...
    if let Some(s) = &o.serial {
        m.bus.serial_input(s.as_bytes());
    }
    for pre in &o.trace_fns {
        let mut n = 0;
        for (&a, name) in m.symbols.clone().iter() {
    ...
    match boot.as_str() {
        "app" => match m.boot_app(0x10000) {
            Ok(entry) => eprintln!("[emu] app boot: entry {:#010x} {}", entry, m.sym(entry)),
            Err(e) => {
                eprintln!("[emu] {}", e);
                std::process::exit(2)
            }
    ...
    if let Some(c) = o.reset_cause {
        m.bus.set_reset_cause(c);
    }
    if let Some(v) = o.strap {
        m.bus.set_strap(v);
    }
    for &(a, n) in &o.peeks {
    ...
    m.console.mask = match console.as_str() {
        "usb" => 1,
        "uart0" => 2,
        "uart" => 2,
        "both" => 3,
    ...
    if let Some(p) = &o.regtrace {
        m.add_observer(Box::new(RegTrace::new(
    ...
    if o.profile {
        m.add_observer(Box::new(PcHist::new(12)));
    }
    if let Some(wa) = o.watch {
        let v = m.bus.read32(wa).unwrap_or(0);
        m.add_observer(Box::new(Watch { addr: wa, value: v }));
    }
    if o.profile_blocks {
        m.add_observer(Box::new(BlockProfile::new(20)));
    }
    if let Some(path) = &o.coverage {
        m.add_observer(Box::new(Coverage::new(path.clone())));
    }
    if o.irq_latency {
        m.add_observer(Box::new(IrqLatency::new(S::CORES)));
    }
    if let Some(p) = &o.vcd {
        m.add_observer(Box::new(Vcd::new(p, S::CPU_HZ)));
    }
    if let Some(p) = &o.script {
        m.load_script(&std::fs::read_to_string(p).expect("script"))
            .expect("script");
    }
    if let Some(sec) = o.max_seconds {
        m.max_cycles = (sec * S::CPU_HZ as f64) as u64;
    }
    ...
    let total = m.insns();
    let per: String = if S::CORES == 1 {
        format!("{}", total)
    } else {
        m.cores
            .iter()
            .enumerate()
            .map(|(i, c)| format!("core{} {}", i, c.insn_count()))
            .collect::<Vec<_>>()
            .join(" + ")
    };
    ...
    if let Stop::Unimplemented(pc, raw) = stop {
        if let Ok(b) = m.bus.fetch(pc) {
            eprintln!(
                "[emu] unimplemented at {:08x} {}: {} (raw {:#x})",
                pc,
                m.sym(pc),
                m.cores[0].disasm(pc, b),
                raw
            );
        }
    }
    ...
    {
        let st: Vec<(u64, u64, u64, usize)> = m
            .cores
            .iter()
            .filter_map(|c| c.code_cache_stats())
            .collect();
        if st.iter().any(|s| s.0 > 0) {
            let b0 = st[0];
            let b1 = st.get(1).copied().unwrap_or((0, 0, 0, 0));
            eprintln!("[emu] blocks: {} built ({} cache flushes) core0, {} ({}) core1; jit: {} compiled, {} KB code", b0.0, b0.1, b1.0, b1.1, b0.2 + b1.2, (b0.3 + b1.3) / 1024);
        }
    }
    if m.stub_hits > 0 {
        eprintln!("[emu] stubs hit {} times", m.stub_hits);
    }
    if let Some(p) = &o.tft_png {
        match m.write_tft_png(p, 3) {
            Ok(()) => eprintln!("[emu] wrote {}", p),
            Err(e) => eprintln!("[emu] png: {}", e),
        }
    }
    if let Some(p) = &o.gram_png {
