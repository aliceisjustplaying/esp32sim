...
impl Emu {
    ...
    fn s3(&mut self) -> Option<&mut Machine> { match &mut self.chip { Chip::S3(m) => Some(m), _ => None } }
    fn text_msg(&mut self, s: &str) { self.queue.push((1, s.as_bytes().to_vec())); }
}
...
pub unsafe extern "C" fn esp32sim_new(board: *const u8, board_len: usize, flash_mb: u32, psram_mb: u32) -> *mut Emu {
    ...
    if board == "esp32c3" || board == "c3" {
        ...
        return Box::into_raw(Box::new(Emu { chip: Chip::C3(m), out: Vec::new(), queue: Vec::new(), booted: false }));
    }
    ...
    Box::into_raw(Box::new(Emu { chip: Chip::S3(m), out: Vec::new(), queue: Vec::new(), booted: false }))
}
...
pub unsafe extern "C" fn esp32sim_load(e: *mut Emu, kind: u32, ptr: *const u8, len: usize) -> u32 {
    ...
    let r = match &mut e.chip {
        ...
        Chip::C3(m) => match kind {
            ...
            4 => m.add_symbols(d),
            5 => m.write_flash(0, d),
            _ => Err(format!("load kind {} is not supported on the C3 (no scripts, no camera)", kind)),
        },
    };
...
pub unsafe extern "C" fn esp32sim_load_at(e: *mut Emu, offset: u32, ptr: *const u8, len: usize) -> u32 {
    let e = &mut *e; let d = bytes(ptr, len);
    let r = match &mut e.chip { Chip::S3(m) => m.write_flash(offset as usize, d), Chip::C3(m) => m.write_flash(offset as usize, d) };
...
pub unsafe extern "C" fn esp32sim_wifi(e: *mut Emu, spec: *const u8, len: usize) {
    let e = &mut *e;
    if e.s3().is_none() { log("[emu] wifi: the C3 radio is not modelled"); return; }
...
pub unsafe extern "C" fn esp32sim_stub(e: *mut Emu, name: *const u8, len: usize, value: u32) -> u32 {
    ...
    let Some(m) = e.s3() else { log("[emu] stub: not supported on the C3"); return 1 };
    match by_addr.or_else(|| m.symbols.iter().find(|(_, n)| n.as_str() == name.as_str()).map(|(a, _)| *a)) {
...
pub unsafe extern "C" fn esp32sim_boot(e: *mut Emu, app_direct: u32) -> u32 {
    ...
    let name = match &mut e.chip {
        Chip::S3(m) => {
            if app_direct != 0 { if let Err(msg) = m.boot_app(0x10000) { log(&format!("[emu] boot: {}", msg)); return 1; } } else { m.boot_rom(); }
            let n = m.bus.board.name().to_string();
            if let Some(w) = &m.web { w.send_text(&format!("{{\"t\":\"board\",\"name\":\"{}\"}}", n)); }
            n
        }
        Chip::C3(m) => {
            if app_direct != 0 { if let Err(msg) = m.boot_app(0x10000) { log(&format!("[emu] boot: {}", msg)); return 1; } } else { m.boot_rom(); }
    ...
    if matches!(e.chip, Chip::C3(_)) { e.text_msg(&format!("{{\"t\":\"board\",\"name\":\"{}\"}}", name)); }
    e.booted = true; 0
}
            ...
            loop {
                match m.run(u64::MAX) {
                    ...
                    Stop::SwReset => {
                        ...
                        let note = format!("[emu] chip reset at t={:.3}s: cause {:#x} ({})", m.bus.cycles as f64 / esp32s3::periph::CPU_HZ as f64, cause, esp32s3::periph::reset_cause_name(cause));
                        log(&note);
                        if let Some(w) = &m.web { w.send_text(&format!("{{\"t\":\"emu\",\"msg\":\"{}\"}}", esp32s3::web::json_escape(&note))); }
                        m.reboot();
                    }
        ...
        Chip::C3(m) => {
            ...
            let rc = loop {
                match m.run(u64::MAX) {
                    ...
                    S::SwReset => {
                        let cause = m.bus.periph.rtc.reset_cause;
                        note = Some(format!("[emu] chip reset at t={:.3}s: cause {:#x} ({})", m.seconds(), cause, esp32s3::periph::reset_cause_name(cause)));
                        m.reboot();
                    }
                    S::Ebreak(pc) => { log(&format!("[emu] ebreak at {:08x} {}", pc, m.sym(pc))); break 3; }
            ...
            if !out.is_empty() {
                ...
                e.text_msg(&format!("{{\"t\":\"serial\",\"src\":\"usb\",\"data\":\"{}\"}}", esp32s3::web::json_escape(&txt)));
            }
            if let Some(n) = note { log(&n); e.text_msg(&format!("{{\"t\":\"emu\",\"msg\":\"{}\"}}", esp32s3::web::json_escape(&n))); }
            let (t, insns) = match &e.chip { Chip::C3(m) => (m.seconds(), m.cpu.insn_count), _ => (0.0, 0) };
...
#[no_mangle] pub unsafe extern "C" fn esp32sim_cpu_hz(e: *mut Emu) -> f64 {
    match &(&*e).chip { Chip::S3(_) => esp32s3::periph::CPU_HZ as f64, Chip::C3(_) => esp32c3::periph::CPU_HZ as f64 }
}
#[no_mangle] pub unsafe extern "C" fn esp32sim_cycles(e: *mut Emu) -> f64 { match &(&*e).chip { Chip::S3(m) => m.bus.cycles as f64, Chip::C3(m) => m.bus.cycles as f64 } }
#[no_mangle] pub unsafe extern "C" fn esp32sim_insns(e: *mut Emu) -> f64 { match &(&*e).chip { Chip::S3(m) => (m.cpu.insn_count + m.cpu1.insn_count) as f64, Chip::C3(m) => m.cpu.insn_count as f64 } }
...
pub unsafe extern "C" fn esp32sim_out_take(e: *mut Emu) -> u32 {
    ...
    if let Chip::S3(m) = &e.chip { if let Some(w) = &m.web { e.out.extend(w.take_outbox()); } }
    e.out.len() as u32
}
...
pub unsafe extern "C" fn esp32sim_in_text(e: *mut Emu, ptr: *const u8, len: usize) {
    ...
    match &mut e.chip {
        Chip::S3(m) => { if let Some(w) = &m.web { w.push_incoming(msg); } }
        ...
        Chip::C3(m) => {
            if esp32s3::web::json_str(&msg, "t").as_deref() == Some("serial") {
                ...
                m.bus.periph.usb.host_input(format!("{}\n", line).as_bytes());
            }
        }
    }
}
...
pub unsafe extern "C" fn esp32sim_in_bin(e: *mut Emu, ptr: *const u8, len: usize) {
    let e = &mut *e;
    if let Chip::S3(m) = &e.chip { if let Some(w) = &m.web { w.push_incoming_bin(bytes(ptr, len).to_vec()); } }
}
