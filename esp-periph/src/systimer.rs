...
impl Device for Systimer {
    fn read(&mut self, off: u32) -> u32 {
        Systimer::read(self, off)
    }
    fn write(&mut self, off: u32, v: u32) -> WriteEffect {
        Systimer::write(self, off, v);
        WriteEffect::NONE
    }
    fn irq_sources(&self) -> u64 {
        (0..3).fold(0, |m, t| m | ((self.irq(t) as u64) << t))
    }
    fn clock(&self) -> Option<ClockDomain> {
        Some(ClockDomain::Systimer)
    }
    fn tick(&mut self, ticks: u64) {
        Systimer::tick(self, ticks)
    }
    ...
    fn next_deadline(&self) -> Option<u64> {
        ...
        for t in 0..3 {
            ...
            let unit = ((self.target_conf[t] >> 31) & 1) as usize;
            if self.conf & (1 << (30 - unit as u32)) == 0 {
                continue;
