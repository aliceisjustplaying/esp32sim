...
pub fn aes_block(key: &[u8], block: &[u8; 16], decrypt: bool) -> [u8; 16] {
    ...
    let rk = |round: usize| -> [u8; 16] {
        let mut k = [0u8; 16];
        for c in 0..4 {
            k[4 * c..4 * c + 4].copy_from_slice(&w[4 * round + c]);
        }
        k
    };
    let inv = inv_sbox();
        ...
        for round in 1..=nr {
            ...
            if round != nr {
                for c in 0..4 {
                    ...
                    for r in 0..4 {
                        s[4 * c + r] = col[r] ^ x ^ xtime(col[r] ^ col[(r + 1) % 4]);
                    }
                }
            }
            let k = rk(round);
...
pub fn aes128_encrypt_block(key: &[u8; 16], block: &[u8; 16]) -> [u8; 16] {
    ...
    for (round, round_key) in rk.iter().enumerate().skip(1) {
        ...
        for (byte, key_byte) in s.iter_mut().zip(round_key) {
            *byte ^= key_byte;
        }
    }
    s
...
mod tests {
    ...
    fn aes_all_key_lengths_roundtrip() {
        ...
        for (klen, ct) in [
            ...
            assert_eq!(
                aes_block(&key, &enc, true)[..],
                pt[..],
                "decrypt with {}-byte key",
                klen
            );
        }
    }
    ...
    #[test]
    fn aes_and_keywrap_rfc_vectors() {
        // FIPS-197 AES-128 and RFC 3394 section 4.1
        let key = hex("000102030405060708090a0b0c0d0e0f");
        let pt = hex("00112233445566778899aabbccddeeff");
        let (mut k, mut b) = ([0u8; 16], [0u8; 16]);
        k.copy_from_slice(&key);
        b.copy_from_slice(&pt);
        assert_eq!(
            aes128_encrypt_block(&k, &b)[..],
...
pub fn bn_divmod(u_in: &[u32], v_in: &[u32]) -> (Vec<u32>, Vec<u32>) {
    ...
    if bn_cmp(&u, &v) == std::cmp::Ordering::Less {
        return (vec![0], u);
    }
    if v.len() == 1 {
        let d = v[0] as u64;
...
pub fn bn_mod(a: &[u32], m: &[u32]) -> Vec<u32> {
    bn_divmod(a, m).1
}
...
pub fn bn_modexp(x: &[u32], y: &[u32], m: &[u32]) -> Vec<u32> {
    ...
    let top = y.len() * 32
        - y.last()
            .expect("the exponent has at least one limb")
