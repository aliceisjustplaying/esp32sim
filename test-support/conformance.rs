...
fn parse_sidecar(path: &Path) -> BTreeMap<String, String> {
    ...
    for (line_number, line) in text.lines().enumerate() {
        ...
        let (key, value) = line.split_once('=').unwrap_or_else(|| {
            panic!("{}:{}: expected key=value", path.display(), line_number + 1)
        });
        assert!(
...
pub fn verify_provenance(
    ...
    let source = sidecar.parent().unwrap().join(&fields["source"]);
    let source_bytes =
        std::fs::read(&source).unwrap_or_else(|e| panic!("{}: {e}", source.display()));
    assert_eq!(
